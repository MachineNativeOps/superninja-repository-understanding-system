...
class TestAgentSystemConversion:
    ...
    def test_base_agent_import(self):
        """測試 BaseAgent 導入"""
        from autonomous.agents.base_agent import AgentStatus, BaseAgent
...
class TestNamingConventions:
    ...
    def test_agent_file_names_are_kebab_case(self):
        ...
        for file in agent_files:
            ...
            assert (
                file.name.islower() or "-" in file.name
...
class TestDirectoryStructure:
    ...
    def test_duplicate_legacy_dirs_removed(self):
        ...
        ).exists(), "Duplicate archive/v1-python-drones should be removed"
        assert not (
            archive_dir / "v2-multi-islands"
        ).exists(), "Duplicate archive/v2-multi-islands should be removed"

        # 檢查原始目錄是否仍然存在
        assert (
            archive_dir / "legacy" / "v1-python-drones"
        ).exists(), "Original legacy/v1-python-drones should still exist"
        assert (
            archive_dir / "legacy" / "v2-multi-islands"
        ).exists(), "Original legacy/v2-multi-islands should still exist"
