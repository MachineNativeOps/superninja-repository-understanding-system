...
def test_estimate_task_cost_success():
    ...
    # Expected cost: shots * cost_per_shot + depth * cost_per_depth
    assert cost == (100 * 0.0001) + (5 * 0.001)
...
def test_optimize_schedule_failure(mock_scheduler):
    ...
    mock_scheduler_instance = mock_scheduler.return_value
    mock_scheduler_instance.rust_scheduler.schedule_tasks.side_effect = Exception(
        "Rust scheduler error"
    )
