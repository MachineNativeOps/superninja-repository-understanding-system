...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/naming-governance-migration.log"),
...
class NamingGovernanceMigration:
    ...
    def __init__(self, config_path: str = None):
        """初始化遷移系統"""
        self.config_path = config_path or "/etc/naming-governance/migration-config.yaml"
    ...
    def _create_backup_point(self) -> str:
        ...
        backup_path = f"/var/backups/naming-governance/migration_{timestamp}"
    ...
    def _generate_dry_run_report(
        ...
        # 保存報告
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = f"/var/log/naming-governance/dry_run_report_{timestamp}.json"
    ...
    def _generate_cutover_report(self, plan: MigrationPlan):
        ...
        # 保存報告
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = f"/var/log/naming-governance/cutover_report_{timestamp}.json"
