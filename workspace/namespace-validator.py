...
class NamespaceValidator:
    ...
    def _validate_yaml_structure(self, doc: dict, violations: List[str]):
        ...
        if "apiVersion" in doc:
            ...
            # Note: Checking for forbidden pattern, not URL sanitization
        ...
        if "kind" in doc:
            kind = doc["kind"]
            if "Axiom" in kind:
                violations.append(f"❌ kind uses forbidden 'Axiom' prefix: {kind}")
    ...
    def _validate_labels(self, labels: dict, violations: List[str]):
        ...
        # Note: Checking for forbidden pattern in metadata, not URL
        # sanitization (CodeQL false positive)
