            ...
            if not semantic_root_ref:
                self.results.append(
                    ValidationResult(
                        level="semantic",
                        status="warning",
                        message=f"No semantic root reference found",
                        details={
                            "path": artifact_path,
                ...
                if semantic_root_ref != semantic_root_version:
                    self.results.append(
                        ValidationResult(
                            level="semantic",
                            status="warning",
                            message=f"Semantic root version mismatch",
                            details={
                                "path": artifact_path,
...
Examples:
  ...
  %(prog)s --level all artifact1.yaml artifact2.yaml
    ...
    if missing_files:
        print(f"❌ Error: Artifact files not found:")
        for f in missing_files:
            print(f"   - {f}")
        sys.exit(1)

    # Create validator
    print(f"🚀 Artifact Validation Tool v2.0.0")
