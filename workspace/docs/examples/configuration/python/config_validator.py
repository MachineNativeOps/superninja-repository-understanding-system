...
class ConfigValidator:
    ...
    def validate_yaml_files(self) -> dict:
        ...
        for yaml_file in yaml_files:
            try:
                ...
                results["errors"].append({"file": yaml_file, "error": str(e)})
    ...
    def validate_json_files(self) -> dict:
        ...
        for json_file in json_files:
            ...
            try:
                ...
                results["invalid_files"] += 1
                results["errors"].append({"file": json_file, "error": str(e)})
            ...
            try:
                ...
                if "version" not in config:
                    results["warnings"].append(
                        {"file": filename, "message": "Missing version field"}
                    )

                # 檢查services
                if "services" not in config:
                    results["errors"].append(
                        {"file": filename, "message": "Missing services section"}
                    )
                    results["valid"] = False
                else:
                    # 檢查每個服務
    ...
    def validate_kubernetes_manifests(self) -> dict:
        """驗證Kubernetes配置"""
        k8s_files = self._find_files(
            ["*.yaml", "*.yml"], directories=["k8s", "kubernetes", ".kube"]
        )
        ...
        required_fields = {"apiVersion": str, "kind": str, "metadata": dict}
        ...
        for k8s_file in k8s_files:
            try:
                ...
                results["errors"].append({"file": k8s_file, "error": str(e)})
                results["invalid_files"] += 1
    ...
    def validate_env_files(self) -> dict:
        ...
        results = {"total_files": len(env_files), "issues": []}
        ...
        for env_file in env_files:
            try:
                ...
                results["issues"].append({"file": env_file, "error": str(e)})
    ...
    def _find_files(self, patterns: list[str], directories: list[str] | None = None) -> list[str]:
        ...
        for search_dir in search_dirs:
            ...
            full_search_dir = (
                ...
                else os.path.join(self.project_path, search_dir)
            )
    ...
    def _generate_validation_summary(self, validations: dict) -> dict:
        ...
        for _validation_name, validation_result in validations.items():
