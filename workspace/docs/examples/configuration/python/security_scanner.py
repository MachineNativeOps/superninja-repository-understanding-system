...
def validate_project_path(input_path: str) -> str:
    """
    Validate and sanitize project path input.
    ...
    Raises:
        ValueError: If path is invalid, contains forbidden characters,
                   or is outside the current working directory
    """
    base_dir = pathlib.Path.cwd().resolve()
    # Forbid shell metacharacters and control characters (expand as needed for additional security)
    # Includes: control chars (0x00-0x1f), shell metacharacters, and redirects
    forbidden = re.compile(r"[\x00-\x1f;&|$`><]")
    if forbidden.search(input_path):
        raise ValueError("Project path contains forbidden characters")
    try:
        resolved_path = pathlib.Path(input_path).expanduser().resolve()
...
class SecurityScanner:
    ...
    def run_bandit_scan(self) -> dict:
        ...
        cmd = [
            ...
            "json",
            "-o",
            os.path.join(self.reports_dir, "bandit-report.json"),
            "--exclude",
            "*/test*,*/venv/*,*/.git/*",
        ]
    ...
    def run_npm_audit(self) -> dict:
        ...
        try:
            ...
            summary = {
                "tool": "npm-audit",
                "timestamp": datetime.now().isoformat(),
                "total_vulnerabilities": report.get("metadata", {})
                .get("vulnerabilities", {})
                .get("total", 0),
    ...
    def _generate_summary(self, scans: dict) -> dict:
        ...
        for _scan_name, scan_result in scans.items():
