...
def validate_project_path(path: str) -> str:
    """Validate and return absolute project path.

    Args:
        path: Project path to validate (can be relative or absolute)

    Returns:
        Absolute path to the validated project directory

    Raises:
        SystemExit: Exit code 2 if validation fails
            - Path is not a directory
            - Path is a symlink
            - Path contains forbidden shell metacharacters
    """
    # Disallow suspicious characters in the path (e.g., shell metacharacters)
    if re.search(r"[`;$|&><`]", path):
        print(f"Error: Project path '{path}' contains forbidden characters.")
        sys.exit(2)

    abs_path = os.path.abspath(path)

    # Check if the path is a valid, non-symlink directory
    if (not os.path.isdir(abs_path)) or os.path.islink(abs_path):
        # Exit code 2: Invalid project path (not a directory or is a symlink)
        print(f"Error: Provided project path '{abs_path}' is not a valid directory.")
        sys.exit(2)
