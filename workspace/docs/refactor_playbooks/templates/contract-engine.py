...
class ContractEngine:
    ...
    async def _execute_validation_gate(
        self, gate: ValidationGate, context: ExecutionContext
    ) -> ValidationResult:
        ...
        try:
            result = await asyncio.wait_for(
                gate.validator_func(context), timeout=gate.timeout_ms / 1000.0
            )
    ...
    async def _execute_contract_actions(self, context: ExecutionContext) -> bool:
        ...
        for action_def in contract.actions:
            ...
            try:
                ...
                self.logger.error(
                    f"Action execution failed: {action_type}", extra={"error": str(e)}
                )
                return False
    ...
    async def _execute_action(
        self, action_type: str, parameters: Dict[str, Any], context: ExecutionContext
    ):
...
async def main():
    ...
    if contract:
        result = await engine.execute_contract(contract.contract_id, user_context)
