...
class TechCapabilityAssessment:
    ...
    def get_gaps(self) -> List[str]:
        """獲取能力缺口"""
        return [name for name, cap in self.capabilities.items() if cap.get_score() < 50]
...
class MarketTimingAnalysis:
    ...
    window_duration_months: Optional[int]  # 機會窗口
...
class StrategyAdvisor:
    ...
    def assess_capabilities(
        self, team_profile: Dict[str, Any], focus_areas: Optional[List[str]] = None
    ...
    def analyze_market_timing(
        self, market_name: str, market_data: Optional[Dict] = None
    ...
    def _calculate_timing_score(
        self, data: Dict, maturity: MarketMaturity, competitors: List[CompetitorProfile]
    ) -> float:
        """計算市場進入時機分數"""
        score = 50.0  # 基礎分
    ...
    def generate_recommendations(
        ...
        if overall_score < 50:
            # 能力不足 → 建設基礎
            recommendations.append(
                self._create_foundation_strategy(capability_assessment, constraints)
            )
        ...
        if "innovation" in [g.lower() for g in business_goals]:
            # 追求創新 → 技術創新策略
            recommendations.append(
                self._create_innovation_strategy(capability_assessment, constraints)
            )

        if "profitability" in [g.lower() for g in business_goals]:
            # 追求獲利 → 效率優化策略
            recommendations.append(
                self._create_efficiency_strategy(capability_assessment, constraints)
            )
    ...
    def _create_expansion_strategy(
        ...
        return StrategyRecommendation(
            ...
            prerequisites=["完成核心產品開發", "建立基礎客戶群", "確保資金到位"],
            expected_outcomes=["市場份額提升 20%", "營收增長 50%", "品牌知名度提升"],
    ...
    def _create_foundation_strategy(
        self, cap: TechCapabilityAssessment, constraints: Dict
    ) -> StrategyRecommendation:
        """創建基礎建設策略"""
        gaps = cap.get_gaps()
    ...
    def _create_growth_strategy(
        self, market: MarketTimingAnalysis, constraints: Dict
    ) -> StrategyRecommendation:
        """創建增長策略"""
        ...
        return StrategyRecommendation(
            ...
            prerequisites=["產品市場契合驗證", "銷售流程標準化", "客戶成功體系建立"],
            expected_outcomes=["用戶數量翻倍", "ARR 增長 100%", "NRR > 120%"],
            estimated_timeline_months=12,
            estimated_investment={
                "sales": budget * 0.35,
    ...
    def _create_innovation_strategy(
        ...
        return StrategyRecommendation(
            ...
            prerequisites=["創新文化建立", "研發資源保障", "容錯機制設計"],
            expected_outcomes=["推出創新功能/產品", "建立技術護城河", "提升產業影響力"],
            estimated_timeline_months=18,
            estimated_investment={
                "r_and_d": budget * 0.5,
                "talent": budget * 0.25,
    ...
    def _create_efficiency_strategy(
        ...
        return StrategyRecommendation(
            ...
            prerequisites=["清楚的成本結構分析", "流程效率評估", "自動化機會識別"],
            expected_outcomes=["營運成本降低 20%", "利潤率提升", "現金流改善"],
