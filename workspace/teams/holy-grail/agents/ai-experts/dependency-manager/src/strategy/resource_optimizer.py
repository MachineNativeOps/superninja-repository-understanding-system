...
class BudgetAllocation:
    ...
    def get_summary(self) -> Dict[str, float]:
        """獲取分配摘要"""
        return {name: cat.amount for name, cat in self.allocations.items()}
...
class ResourceOptimizer:
    ...
    def optimize_budget(
        ...
        template = self.BUDGET_TEMPLATES.get(
            strategy, self.BUDGET_TEMPLATES[AllocationStrategy.BALANCED]
        )
    ...
    def _calculate_budget_optimization_score(
        self, allocations: Dict[str, BudgetCategory], strategy: AllocationStrategy
    ) -> float:
        """計算預算優化分數"""
        # 基於有效價值計算分數
    ...
    def optimize_team(
        ...
        Returns:
            優化後的團隊配置
        """
        template = self.TEAM_TEMPLATES.get(stage, self.TEAM_TEMPLATES["startup"])
        ...
        return TeamAllocation(
            ...
            structure=(
                "flat"
                if actual_headcount < 10
                else "hierarchical" if actual_headcount < 50 else "matrix"
            ),
            optimization_score=optimization_score,
    ...
    def _calculate_team_optimization_score(
        self, roles: Dict[str, TeamRole], target: int, budget: float, actual_cost: float
    ) -> float:
        """計算團隊優化分數"""
        score = 50.0  # 基礎分
    ...
    def generate_optimization(
        ...
        budget_allocation = self.optimize_budget(operational_budget, strategy)

        team_allocation = self.optimize_team(target_headcount, personnel_budget, stage)
    ...
    def _calculate_projected_roi(
        self, budget: BudgetAllocation, team: TeamAllocation, goals: Optional[List[str]]
    ) -> float:
        """計算預測 ROI"""
        # 基於預算分配的預期回報
        total_expected_return = sum(
            cat.amount * cat.roi_multiplier for cat in budget.allocations.values()
        )
    ...
    def _calculate_payback_period(self, investment: float, roi_percent: float) -> int:
        """計算回收期（月）"""
        if roi_percent <= 0:
            return 120  # 10 年
    ...
    def _perform_sensitivity_analysis(
        self, budget: float, headcount: int, base_roi: float
    ) -> Dict[str, Any]:
        """執行敏感度分析"""
        return {
            "budget_sensitivity": {
    ...
    def _generate_recommendations(
        ...
        if goals:
            ...
            if "innovation" in [g.lower() for g in goals]:
                if (
                    "r_and_d" not in budget.allocations
                    or budget.allocations["r_and_d"].amount < budget.total_budget * 0.15
                ):
