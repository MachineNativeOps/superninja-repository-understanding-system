...
class PhoenixAgent:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if self.config_path.exists():
            ...
            return {
                ...
                "escalation": {"enabled": True, "levels": [1, 2, 3, 4, 5]},
            }
            ...
            try:
                ...
                if cmdline and "automation_launcher.py" in " ".join(cmdline):
                    ...
                    return HealthCheck(
                        ...
                        status=status,
                        timestamp=timestamp,
                        details={"pid": proc.pid, "running": True},
                        metrics={"cpu_percent": cpu_percent, "memory_mb": memory_mb},
                    )
    ...
    async def _handle_new_incident(self, component: str, health: HealthCheck):
        ...
        self.logger.warning(
            f"🚨 New incident: {incident_id} - {component} is {health.status.value}"
        )
    ...
    def _select_recovery_strategy(
        self, incident: Incident, health: HealthCheck
    ) -> RecoveryStrategy:
    ...
    async def _execute_recovery(
        self, incident: Incident, strategy: RecoveryStrategy
    ) -> RecoveryAttempt:
    ...
    async def _safe_mode_restart(self, component: str) -> bool:
        ...
        try:
            ...
            safe_mode_marker.write_text(
                f"safe_mode={component}\ntimestamp={datetime.now().isoformat()}\n"
            )
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if cmdline and component in " ".join(cmdline):
                        self.logger.info(
                            f"Terminating {component} (PID: {proc.pid}) for safe mode restart"
                        )
                        proc.terminate()
                        proc.wait(timeout=10)
    ...
    async def _full_system_bootstrap(self, component: str) -> bool:
        ...
        try:
            ...
            bootstrap_log = (
                BASE_PATH
                / ".automation_logs"
                / f"bootstrap_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            )
