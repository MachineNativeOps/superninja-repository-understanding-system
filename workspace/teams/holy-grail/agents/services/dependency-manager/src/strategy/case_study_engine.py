...
class CaseStudy:
    ...
    def get_phase_by_type(self, phase_type: PhaseType) -> EvolutionPhase | None:
        """根據類型獲取階段"""
...
class CaseStudyEngine:
        ...
        self._case_studies["shopify"] = CaseStudy(
            ...
            lessons_learned=[
                LessonLearned(
                    ...
                    common_mistakes=["功能過於複雜", "定價策略混亂", "缺乏引導流程"],
                ),
                LessonLearned(
                    category="生態建設",
                    title="開放平台創造網絡效應",
    ...
    def get_case(self, company: str) -> CaseStudy | None:
        """獲取特定公司案例"""
    ...
    def find_cases_by_strategy(
        self, strategy: DevelopmentStrategy
    ) -> list[tuple[CaseStudy, EvolutionPhase]]:
        """根據策略查找案例"""
        results = []
        for case in self._case_studies.values():
    ...
    def analyze_strategy_patterns(self) -> dict[str, Any]:
        ...
        for case in self._case_studies.values():
            ...
            sequences.append({"company": case.company_name, "sequence": seq})
