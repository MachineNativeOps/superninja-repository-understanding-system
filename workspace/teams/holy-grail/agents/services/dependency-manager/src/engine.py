...
from .analyzers import BaseAnalyzer, GoAnalyzer, NpmAnalyzer, PipAnalyzer
...
from .scanners import LicenseScanner, VulnerabilityScanner
from .scanners.license_scanner import LicenseScanResult
...
class ManagerConfig:
    ...
    ecosystems: list[Ecosystem] = field(
        default_factory=lambda: [Ecosystem.NPM, Ecosystem.PIP, Ecosystem.GO]
    )
...
class DependencyManager:
    ...
    def _load_config(self, config_path: str | None) -> ManagerConfig:
        """
        載入配置
    ...
    async def analyze_project(
        self, project_path: str, scan_type: str = "full"
    ...
    async def scan_licenses(self, analysis: DependencyAnalysis) -> LicenseScanResult:
        """
        掃描許可證
    ...
    async def update_dependencies(
        self, analysis: DependencyAnalysis, security_only: bool = False
    ) -> UpdateResult:
        """
        更新依賴項
        ...
        # 獲取需要安全修復的套件
        security_fixes = [d.name for d in analysis.dependencies if d.has_vulnerability]
    ...
    async def full_scan(self, project_path: str) -> dict[str, Any]:
        """
        執行完整掃描
