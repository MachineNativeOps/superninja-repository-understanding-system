...
class SuccessMetricsTracker:
    ...
    def _initialize_default_metrics(self) -> None:
        ...
        # 組織指標
        self.add_organizational_metric(
    ...
    def record_value(
        self, category: MetricCategory, metric_id: str, value: float, notes: str = ""
    ) -> bool:
        """記錄指標值"""
    ...
    def get_critical_metrics(self) -> list[dict[str, Any]]:
        ...
        for m in self.technical_metrics.values():
            if m.get_status() == MetricStatus.CRITICAL:
                critical.append({"category": "technical", **m.to_dict()})

        for m in self.business_metrics.values():
            if m.get_status() == MetricStatus.CRITICAL:
                critical.append({"category": "business", **m.to_dict()})

        for m in self.organizational_metrics.values():
            if m.get_status() == MetricStatus.CRITICAL:
                critical.append({"category": "organizational", **m.to_dict()})
    ...
    def _generate_markdown_report(self) -> str:
        ...
        for m in self.technical_metrics.values():
            status_icon = {"on_target": "✅", "warning": "⚠️", "critical": "❌"}[
                m.get_status().value
            ]
            ...
            lines.append(
                f"| {m.name} | {m.target_value} {m.unit} | {current_str} {m.unit} | {status_icon} | {trend_icon} |"
            )
        ...
        for m in self.business_metrics.values():
            ...
            lines.append(
                f"| {m.name} | {m.target_value} {m.unit} | {current_str} {m.unit} | {status_icon} | {growth_str} |"
            )
        ...
        for m in self.organizational_metrics.values():
            ...
            current_str = f"{current:.1f}" if current is not None else "N/A"
            lines.append(
                f"| {m.name} | {m.target_value} {m.unit} | {current_str} {m.unit} | {status_icon} |"
            )
    ...
    def _generate_text_report(self) -> str:
        ...
        for m in self.technical_metrics.values():
            ...
            current_str = f"{current:.1f}" if current is not None else "N/A"
            lines.append(
                f"  {m.name}: {current_str}/{m.target_value} {m.unit} [{m.get_status().value}]"
            )

        lines.extend(["", "商業指標：", "-" * 30])
