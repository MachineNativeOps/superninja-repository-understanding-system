...
class DAppAssessment:
    ...
    recommended_blockchain: BlockchainType | None = None
...
class Web3Integration:
        ...
        if specs:
            ...
            if specs["tps"] >= assessment.estimated_tps_requirement:
                ...
                assessment.risks.append(
                    f"區塊鏈 TPS ({specs['tps']}) 可能無法滿足需求 ({assessment.estimated_tps_requirement})"
                )
    ...
    def _generate_dapp_recommendations(self, assessment: DAppAssessment) -> list[str]:
        ...
        if assessment.recommended_blockchain != assessment.blockchain:
            recommendations.append(
                f"💡 建議考慮使用 {assessment.recommended_blockchain.value} 替代 {assessment.blockchain.value}"
            )
    ...
    def evaluate_nft_strategy(self, strategy: NFTStrategy) -> NFTStrategy:
        ...
        # Gas 成本估算
        minting_costs = {"lazy": 0.01, "batch": 0.05, "on_demand": 0.02}
        storage_costs = {"ipfs": 0.01, "arweave": 0.05, "centralized": 0.001}
        strategy.estimated_gas_cost = minting_costs.get(
            strategy.minting_approach, 0.02
        ) + storage_costs.get(strategy.storage_type, 0.01)
        ...
        strategy.technical_complexity = complexity_map.get(
            (strategy.marketplace_strategy, strategy.minting_approach), "medium"
        )
    ...
    def evaluate_smart_contract(self, contract: SmartContractDev) -> SmartContractDev:
        """評估智能合約開發"""
        estimates = self.CONTRACT_ESTIMATES.get(
            contract.contract_type, self.CONTRACT_ESTIMATES["custom"]
        )

        contract.estimated_development_hours = estimates["hours"]
        contract.estimated_audit_cost = (
            estimates["audit_cost"] if contract.security_audit_required else 0
        )
    ...
    def _identify_security_risks(self, contract: SmartContractDev) -> list[str]:
        ...
        if contract.contract_type == "defi":
            ...
            risks.extend(["元數據篡改", "鑄造邏輯漏洞", "版稅規避"])
    ...
    def _generate_best_practices(self, contract: SmartContractDev) -> list[str]:
        ...
        if contract.contract_type == "defi":
            practices.extend(["實施時間鎖和多簽機制", "設置合理的滑點保護"])
