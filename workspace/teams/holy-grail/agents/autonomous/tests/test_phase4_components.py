...
class TestAutoGovernanceHub:
    ...
    async def test_process_change_request_low_risk(self, hub):
        ...
        request = ChangeRequest(
            ...
            parameters={"tests_passing": True, "no_breaking_changes": True},
            risk_score=0.2,
        )
    ...
    async def test_auto_corrections(self, hub):
        ...
        request = ChangeRequest(
            ...
            parameters={"rollback_available": False},
            risk_score=0.3,
        )
...
class TestPhase4Integration:
    ...
    async def test_full_autonomous_workflow(self):
        ...
        assert all(r["trust_outcome"] in ["approved_auto", "executed"] for r in results)
        assert all(r["gov_outcome"] in ["approve", "auto_fix"] for r in results)
