...
class VisualizationAgent:
    ...
    async def generate_explanation(
        self, query: str, problem_content: str = "", editor_code: str = ""
    ) -> Dict[str, Any]:
        """
    ...
    def _identify_topic(self, query: str) -> str:
        ...
        if any(
            keyword in query_lower
            for keyword in ["array", "list", "tree", "graph", "stack", "queue", "hash"]
        ):
        ...
        if any(
            keyword in query_lower
            for keyword in ["performance", "optimization", "efficiency", "性能", "優化"]
        ):
            return "performance"
        ...
        if any(
            keyword in query_lower
            for keyword in ["thread", "async", "concurrent", "parallel", "並發", "異步"]
        ):
            return "concurrency"
        ...
        if any(
            keyword in query_lower
            for keyword in ["safety", "reliability", "fault", "error", "安全", "可靠"]
        ):
            return "safety"
    ...
    async def stream_explanation(
        ...
        # Stream summary
        yield {"type": "summary", "data": result["explanation"]["summary"]}
        await asyncio.sleep(0.1)

        # Stream analogy
        yield {"type": "analogy", "data": result["explanation"]["analogy"]}
        await asyncio.sleep(0.1)

        # Stream key points
        for point in result["explanation"]["key_points"]:
            yield {"type": "key_point", "data": point}
            await asyncio.sleep(0.1)

        # Stream example
        yield {"type": "example", "data": result["explanation"]["practical_example"]}

        # Stream follow-up questions
        yield {"type": "follow_up", "data": result["follow_up_questions"]}

        # Complete
        yield {"type": "complete", "data": {"topic": result["topic"]}}
