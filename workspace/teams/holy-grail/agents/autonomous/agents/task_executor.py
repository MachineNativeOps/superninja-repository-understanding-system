...
try:
    ...
    logger.warning(
        "python-dotenv not available, environment variables will not be loaded from .env file"
    )
...
class TaskExecutor:
    ...
    async def execute_auto_fix(self, code: str, issue: Dict[str, Any]) -> Optional[str]:
        """
    ...
    async def stream_analysis(
        ...
        yield {"status": "started", "timestamp": asyncio.get_event_loop().time()}
        ...
        for issue in analysis_result["issues"]:
            yield {"status": "issue_found", "issue": issue}
