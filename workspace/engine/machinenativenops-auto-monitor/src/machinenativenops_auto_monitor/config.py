...
class AutoMonitorConfig:
    ...
    def default(cls) -> "AutoMonitorConfig":
        ...
        return cls(
            ...
            collectors={
                "system": {"enabled": True},
                "service": {"enabled": True, "services": [], "timeout": 5},
            },
            alerts={
                "enabled": True,
                "rules": [
...
def get_default_config() -> MonitorConfig:
    ...
    return {
        ...
        "metrics": {"enabled": True, "applications": []},
        "alerts": {
            "enabled": True,
            "alert_rules": {
                "cpu_threshold": 80.0,
        ...
        "storage": {"type": "memory", "retention_days": 7},
    }
...
def validate_config(config: MonitorConfig) -> bool:
    ...
    for threshold_name, threshold_value in alert_rules.items():
        ...
        if threshold_value < 0 or threshold_value > 100:
            logger.warning(
                f"Threshold {threshold_name} = {threshold_value} is outside normal range [0-100]"
            )
