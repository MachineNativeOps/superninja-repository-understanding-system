...
class FHSStructureValidator:
    def __init__(self, base_path: str = "."):
        ...
        self.fhs_directories = {
            ...
            "etc": {
                "description": "系統配置檔案",
                "required": True,
                "subdirs": ["network", "sysconfig", "cron.d"],
            },
            "lib": {
                "description": "共享函式庫",
                "required": True,
                "subdirs": ["modules", "firmware"],
            },
            "var": {
                "description": "變動資料",
                "required": True,
                "subdirs": ["log", "tmp", "cache", "lib", "spool", "run"],
            },
            "usr": {
                "description": "用戶程式",
                "required": True,
                "subdirs": ["bin", "sbin", "lib", "share", "local"],
            },
        ...
        for dir_name, dir_config in self.fhs_directories.items():
            ...
            if dir_path.exists() and dir_path.is_dir():
                results["fhs_directories"]["valid"].append(
                    {"name": dir_name, "description": dir_config["description"]}
                )
                    ...
                    for subdir in dir_config["subdirs"]:
                        ...
                        if not (subdir_path.exists() and subdir_path.is_dir()):
                            results["fhs_directories"]["invalid_subdirs"].append(
                                {"parent": dir_name, "missing_subdir": subdir}
                            )
            else:
    ...
    def generate_report(self) -> str:
        ...
        report_lines.extend(["", f"## 合規性分數: {compliance_score:.1f}/100"])
