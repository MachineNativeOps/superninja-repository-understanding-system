    ...
    def validate_yaml_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                # YAML 解析錯誤
                self.result.errors.append((str(file_path), f"YAML 解析錯誤: {str(e)}"))
                return False

        except Exception as e:
            self.result.errors.append((str(file_path), f"文件讀取錯誤: {str(e)}"))
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MachineNativeOps 命名空間驗證工具")
    parser.add_argument(
        "directory", nargs="?", default=".", help="要驗證的目錄路徑（默認為當前目錄）"
    )
    parser.add_argument("--staged", action="store_true", help="只驗證 git staged 文件")
