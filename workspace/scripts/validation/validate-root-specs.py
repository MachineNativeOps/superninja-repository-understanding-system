...
class RootSpecsValidator:
    ...
    def load_registries(self):
        ...
        registry_files = ["root.registry.modules.yaml", "root.registry.urns.yaml"]
    ...
    def _validate_yaml_keys(
        self, data: Any, file_name: str, naming_rules: Dict, path: str = ""
    ) -> List[str]:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if not is_exception and key_pattern and not re.match(key_pattern, key):
                    errors.append(
                        f"{file_name}: Key '{current_path}' does not match pattern: {key_pattern}"
                    )
        ...
        if module_registry:
            ...
            for module in modules:
                ...
                if module_name and entrypoint and module_name not in entrypoint:
                    errors.append(
                        f"Module '{module_name}' name not found in entrypoint: {entrypoint}"
                    )
    ...
    def validate_context_spec(self) -> List[str]:
        ...
        if module_registry and modules_config:
            registry_modules = {
                m.get("id"): m.get("name")
                for m in module_registry.get("spec", {}).get("modules", [])
            }
            config_modules = {
                m.get("name"): m.get("name")
                for m in modules_config.get("spec", {}).get("modules", [])
            }

            for module_id, registry_name in registry_modules.items():
                if registry_name not in config_modules:
                    errors.append(
                        f"Module '{registry_name}' in registry not found in root.modules.yaml"
                    )
