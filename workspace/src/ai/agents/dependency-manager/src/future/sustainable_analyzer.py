...
class CarbonFootprint:
    ...
    def _calculate_total(self):
        ...
        self.total_emissions = (
            self.build_emissions + self.runtime_emissions + self.transfer_emissions
        )
...
class EnergyEfficiency:
    ...
    def calculate_energy_cost(self, rate_per_kwh: float = 0.12) -> Dict[str, float]:
        ...
        return {
            ...
            "savings_potential": self.monthly_energy
            * 12
            * rate_per_kwh
            * (self.optimization_potential / 100),
        }
...
class SustainableAnalyzer:
    ...
    EMISSION_FACTORS = {
        ...
        "npm": {"base": 0.001, "per_mb": 0.0002, "per_dep": 0.0001},
        "pip": {"base": 0.0008, "per_mb": 0.00015, "per_dep": 0.00008},
        "go": {"base": 0.0006, "per_mb": 0.0001, "per_dep": 0.00005},
        "cargo": {"base": 0.0005, "per_mb": 0.00008, "per_dep": 0.00004},
        "maven": {"base": 0.002, "per_mb": 0.0003, "per_dep": 0.00015},
    }
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.green_score: Optional[GreenScore] = None
    ...
    def _estimate_cpu_intensity(self, ecosystem: str, size_mb: float) -> float:
        """估算 CPU 密集度"""
        base_intensity = {"npm": 20, "pip": 15, "go": 25, "cargo": 30, "maven": 35}.get(
            ecosystem, 20
        )
    ...
    def analyze_energy_efficiency(
        self, component_name: str, component_type: str, metrics: Dict[str, float]
    ) -> EnergyEfficiency:
        """
        分析能源效率
        ...
        efficiency_score = sum(
            scores[k] * self.EFFICIENCY_WEIGHTS[k] for k in self.EFFICIENCY_WEIGHTS
        )
        ...
        if self.carbon_footprints:
            ...
            if high_impact:
                ...
                for cf in high_impact[:5]:
                    report_lines.append(
                        f"    - {cf.dependency_name} ({cf.total_emissions:.4f} kg CO2e)"
                    )
