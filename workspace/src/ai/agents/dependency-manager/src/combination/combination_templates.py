...
class CombinationTemplate:
    ...
    exploration_prompt: Optional[str]
    ...
    def get_total_allocation(self) -> float:
        return (
            ...
            + sum(self.satellite_prompts.values())
            + self.exploration_allocation
        )
...
class CombinationTemplateManager:
    ...
    def _load_defaults(self):
        self._templates["steady_a"] = CombinationTemplate(
            ...
            exploration_prompt="智能化",
            exploration_allocation=5.0,
            expected_outcomes=["穩固基礎", "客戶信任"],
            risks=["錯失機會"],
        )
        ...
        self._templates["rapid_b"] = CombinationTemplate(
            ...
            exploration_prompt="下世代",
            exploration_allocation=5.0,
            expected_outcomes=["快速驗證", "早期營收"],
            risks=["技術債務"],
        )

    def get_template(self, template_id: str) -> Optional[CombinationTemplate]:
