...
class QuarterlyReviewEngine:
    ...
    def _generate_recommendations(self, review: QuarterlyReview) -> List[str]:
        ...
        for item in review.items:
            if item.achievement_rate < 70:
                recs.append(
                    f"建議加強 {item.metric_name}，目前達成率僅 {item.achievement_rate:.1f}%"
                )
    ...
    def get_review(self, quarter: str) -> Optional[QuarterlyReview]:
    ...
    def _markdown_report(self, review: QuarterlyReview) -> str:
        ...
        for item in review.items:
            lines.append(
                f"| {item.category.value} | {item.metric_name} | {item.target_value}{item.unit} | {item.actual_value}{item.unit} | {item.achievement_rate:.1f}% |"
            )
    ...
    def _text_report(self, review: QuarterlyReview) -> str:
        ...
        for item in review.items:
            lines.append(
                f"  - {item.metric_name}: {item.actual_value}/{item.target_value} ({item.achievement_rate:.1f}%)"
            )
