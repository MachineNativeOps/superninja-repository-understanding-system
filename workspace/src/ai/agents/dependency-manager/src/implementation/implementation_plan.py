...
class Milestone:
    ...
    def _calculate_expected_progress(self) -> float:
        ...
        if not self.tasks:
            return 0.0
        earliest_start = min(
            (t.start_date for t in self.tasks if t.start_date), default=datetime.now()
        )
...
class ImplementationPlan:
    ...
    def _initialize_default_phases(self) -> None:
        ...
        phase1 = Phase(
            ...
            risks=["團隊技能差距過大", "工具鏈整合困難", "核心提示詞選擇錯誤"],
        )
        ...
        for phase in self.phases:
            ...
            if phase.milestones:
                ...
                for milestone in phase.milestones:
                    ...
                    lines.append(
                        f"- {status} **{milestone.name}** - {milestone.target_date.strftime('%Y-%m-%d')} ({milestone.progress_percentage():.0f}%)"
                    )
                lines.append("")
