...
class WorkflowScheduler:
    ...
    def __init__(self):
        ...
        if self.rust_enabled:
            try:
                ...
                logger.warning(
                    f"Failed to initialize Rust scheduler: {str(e)}. Using Python fallback."
                )
                self.rust_scheduler = None
    ...
    def schedule_tasks(
        self, tasks: List[Task], max_latency: float = 600.0, max_budget: float = 100.0
    ) -> List[Task]:
        """
    ...
    def _schedule_with_rust(
        self, tasks: List[Task], max_latency: float, max_budget: float
    ) -> List[Task]:
        ...
        try:
            ...
            prioritized_json = self.rust_scheduler.schedule_tasks(
                json.dumps(task_configs), max_latency=max_latency, max_budget=max_budget
            )
