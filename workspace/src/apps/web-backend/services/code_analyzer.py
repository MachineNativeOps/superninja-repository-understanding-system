...
class AnalysisResult:
    ...
    metrics: CodeMetrics = field(
        default_factory=lambda: CodeMetrics(
            lines_of_code=0,
...
class BaseAnalyzer:
    ...
    def __init__(self, config: dict[str, Any], cache_client: Any | None = None):
        self.config = config
    ...
    async def _perform_analysis(
        self, code: str, file_path: str, strategy: AnalysisStrategy
    ) -> list[CodeIssue]:
        """執行分析 - 由子類實現具體分析邏輯
        ...
        self.logger.warning(
            f"{self.__class__.__name__}._perform_analysis() not implemented for {file_path}"
        )
        return []
...
class CodeAnalysisEngine:
    ...
    def __init__(self, config: dict[str, Any], cache_client: Any | None = None):
        ...
        self.logger = logging.getLogger(self.__class__.__name__)
        self.cache_client = cache_client
        self.analyzers: list[BaseAnalyzer] = [StaticAnalyzer(config, cache_client)]
        self.executor = ThreadPoolExecutor(max_workers=config.get("max_workers", 4))

    async def analyze_file(
        self, file_path: str, strategy: AnalysisStrategy = AnalysisStrategy.STANDARD
    ) -> list[CodeIssue]:
        """
        分析單個文件
...
async def main():
    ...
    print("支持的語言: Python, JavaScript, Go, Rust, Java, C++")
