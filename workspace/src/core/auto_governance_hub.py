...
class AutoGovernanceHub:
    ...
    def _initialize_default_policies(self) -> None:
        ...
        self.add_policy(
            GovernancePolicy(
                ...
                auto_approve_conditions=["within_quota == true", "cost_increase < 20%"],
            )
        )
    ...
    async def _evaluate_policy(
        self, request: ChangeRequest, policy: GovernancePolicy
    ) -> Dict[str, Any]:
    ...
    def _evaluate_condition(self, condition: str, request: ChangeRequest) -> bool:
    ...
    def _determine_action(self, evaluation_results: List[Dict[str, Any]]) -> tuple:
        ...
        return (GovernanceAction.APPROVE, "No blocking conditions found", "default")

    async def _apply_auto_corrections(self, request: ChangeRequest) -> List[str]:
    ...
    async def _make_default_decision(
        self, decision_id: str, request: ChangeRequest
    ) -> GovernanceDecision:
    ...
    async def verify_compliance(self, scope: str = "all") -> ComplianceStatus:
        """
    ...
    async def _auto_fix_violation(
        self, policy: GovernancePolicy, violation: Dict[str, Any]
    ) -> bool:
    ...
    def register_change_handler(
        self, change_type: ChangeType, handler: Callable[..., Awaitable[Any]]
    ) -> None:
    ...
    def get_decision_history(
        self, limit: int = 10, change_type: Optional[ChangeType] = None
    ) -> List[Dict[str, Any]]:
