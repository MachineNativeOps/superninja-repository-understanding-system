    ...
    def _statistical_match(self, actual: Any, expected: Any) -> bool:
        ...
        if isinstance(actual, (int, float)):
            ...
            if max_val is not None and actual > max_val:
                return False
            return not (mean is not None and abs(actual - mean) > mean * tolerance)
            ...
            if mean is not None and abs(actual_mean - mean) > mean * tolerance:
                return False

            if min_val is not None and min(actual) < min_val:
                return False
            return not (max_val is not None and max(actual) > max_val)
