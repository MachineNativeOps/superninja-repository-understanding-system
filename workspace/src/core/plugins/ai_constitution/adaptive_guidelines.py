...
class DomainGuideline:
    ...
    def get_guideline(self, key: str) -> dict[str, Any] | None:
        """獲取特定指南"""
...
class ContextualGuideline:
    ...
    def get_active_guideline(self, key: str) -> Any | None:
        """獲取當前激活的指南"""
...
class LearningGuideline:
    ...
    def _learn_from_data(self, operation_type: str):
        ...
        [d for d in data if not d["outcome"].get("success", True)]
    ...
    def reset_learning(self, operation_type: str | None = None):
        """重置學習數據"""
        if operation_type:
