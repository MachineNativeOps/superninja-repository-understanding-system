...
class CloudProviderAdapter:
    ...
    def is_available(self) -> bool:
        ...
        return self.config.enabled and self._status in (
    ...
    async def execute(self, task: Any) -> ExecutionResult:
        ...
        result = ExecutionResult(
            execution_id=execution_id, provider=self.config.name, status="running"
        )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            self._status = ProviderStatus.UNAVAILABLE
            return {"healthy": False, "status": self._status.value, "error": str(e)}
...
class AzureFunctionsAdapter(CloudProviderAdapter):
    ...
    def __init__(self, name: str, region: str = "eastus", **kwargs):
        if "runtime" not in kwargs:
            kwargs["runtime"] = "node"

        config = ProviderConfig(
            name=name, provider_type=ProviderType.AZURE, region=region, **kwargs
        )
        super().__init__(config)
...
def create_provider_adapter(
    provider_type: ProviderType, name: str, **kwargs
    ...
    if provider_type == ProviderType.AWS:
        ...
        config = ProviderConfig(name=name, provider_type=provider_type, **kwargs)
