...
class ResourceDescriptor:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"uri": self.uri, "digest": self.digest}
        if self.name:
...
class ProvenanceGenerator:
    ...
    def add_dependency(
        self, uri: str, digest: Dict[str, str], name: Optional[str] = None, **kwargs
    ) -> None:
        """
        ...
        dep = ResourceDescriptor(uri=uri, digest=digest, name=name, **kwargs)
    ...
    def generate_provenance(
        ...
        for subj in subjects:
            self._current_build["subjects"].append(
                Subject(name=subj["name"], digest=subj["digest"])
            )

        return self.finish_build()

    def validate_provenance(
        self, provenance: Provenance, target_level: SLSALevel = SLSALevel.L3
    ) -> Dict[str, Any]:
        """
    ...
    def _compute_file_digest(
        self, file_path: str, algorithms: List[DigestAlgorithm] = None
    ) -> Dict[str, str]:
    ...
    def _compute_content_digest(
        self, content: bytes, algorithms: List[DigestAlgorithm] = None
    ) -> Dict[str, str]:
    ...
    def _get_level_from_issues(self, issues: List[str], target_level: SLSALevel) -> int:
...
def create_provenance_generator(
    builder_id: str, builder_version: Optional[str] = None
) -> ProvenanceGenerator:
