...
class AttestationManager:
    ...
    def create_attestation(
        ...
        attestation_subjects = [
            AttestationSubject(name=s["name"], digest=s["digest"], uri=s.get("uri"))
            for s in subjects
        ]
    ...
    async def sign_attestation(
        self, attestation_id: str, identity_token: Optional[str] = None
    ) -> Attestation:
        """
    ...
    def get_attestations_for_artifact(
        self, digest: str, algorithm: str = "sha256"
    ) -> List[Attestation]:
        """
    ...
    def verify_attestation(
        ...
        attestation = self._attestations.get(attestation_id)
        if not attestation:
            return {"valid": False, "error": f"Attestation not found: {attestation_id}"}
        ...
        for subject in attestation.subjects:
            for alg, digest in subject.digest.items():
                ...
                if key in self._subject_index:
                    self._subject_index[key] = [
                        aid for aid in self._subject_index[key] if aid != attestation_id
                    ]
    ...
    def _verify_subjects(
        self, actual: List[AttestationSubject], expected: List[Dict[str, Any]]
    ) -> bool:
...
def create_attestation_manager(
    storage_backend: Optional[Any] = None, signer: Optional[Callable] = None
) -> AttestationManager:
...
def create_provenance_attestation(
    subjects: List[Dict[str, Any]], build_type: str, builder_id: str, **kwargs
) -> Dict[str, Any]:
    """
