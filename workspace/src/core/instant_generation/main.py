...
from .optimization import PerformanceOptimizer, SelfHealingSystem
...
class InstantGenerationSystem:
    ...
    async def generate_system(
        ...
        try:
            ...
            # 記錄性能指標
            await self.tracker.record_generation(
                generation_id,
                {
    ...
    async def _generate_final_output(
        self, workflow_result: Dict[str, Any], generation_id: str
    ) -> Dict[str, Any]:
        """生成最終輸出"""
        ...
        final_output = {
            ...
            "system_overview": {
                ...
                "type": analysis_result.get("output_data", {})
                .get("analysis", {})
                .get("domain", "web"),
