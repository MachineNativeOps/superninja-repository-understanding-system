...
class HallucinationDetector:
        ...
        return ValidationResult(
            is_valid=len(
                [
                    ...
                    if h.severity in [SeverityLevel.CRITICAL, SeverityLevel.HIGH]
                ]
            )
            == 0,
    ...
    def _detect_overconfidence(self, code: str) -> list[HallucinationDetection]:
        ...
        overconfident_patterns = [
            (
                r"#\s*(This is|This code is)\s*(secure|safe|perfect|complete|optimal)",
                "Security/quality claim",
            ),
    ...
    def _filter_false_positives(
        self, detections: list[HallucinationDetection], code: str
    ) -> list[HallucinationDetection]:
    ...
    def _calculate_confidence(self, detections: list[HallucinationDetection]) -> float:
        ...
        total_penalty = sum(
            severity_weights.get(d.severity, 0.1) * d.confidence for d in detections
        )
    ...
    def register_custom_validator(
        self, validator: Callable[[str], list[HallucinationDetection]]
    ) -> None:
