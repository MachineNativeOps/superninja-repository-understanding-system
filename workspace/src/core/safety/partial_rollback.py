...
class PartialRollbackManager:
    ...
    def evaluate_rollback_trigger(self, condition: str, scope: str) -> RollbackAction:
        ...
        action = trigger_map.get(trigger, {}).get(
            rollback_scope, RollbackAction.EMERGENCY_STOP_AND_ROLLBACK
        )
    ...
    def execute_rollback(
        self, scope: str, target: str, execution_id: str | None = None
    ) -> RollbackResult:
        """
    ...
    def cleanup_old_checkpoints(self, execution_id: str, keep_count: int = 5) -> int:
        """
