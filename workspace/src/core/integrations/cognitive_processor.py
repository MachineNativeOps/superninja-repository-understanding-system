...
class PerceptionLayer:
    ...
    async def process(
        self, signal: CognitiveSignal, context: CognitiveContext
...
class ReasoningLayer:
    ...
    async def process(
        self, signals: List[CognitiveSignal], context: CognitiveContext
    ...
    def _requires_decision(self, signal: CognitiveSignal, risk: RiskAssessment) -> bool:
    ...
    async def _make_decision(
        self, signal: CognitiveSignal, risk: RiskAssessment, context: CognitiveContext
    ) -> Decision:
        ...
        requires_approval = (
            risk.level in [RiskLevel.CRITICAL, RiskLevel.HIGH]
            or confidence == DecisionConfidence.LOW
            or confidence == DecisionConfidence.UNCERTAIN
        )
    ...
    def _generate_mitigations(
        ...
        if level == RiskLevel.CRITICAL:
            ...
            mitigations.extend(
                ["Schedule investigation within 24 hours", "Review related system logs"]
            )
...
class ExecutionLayer:
    ...
    async def execute(
        self, decisions: List[Decision], context: CognitiveContext
    ) -> List[CognitiveSignal]:
        """
        ...
        for decision in decisions:
            if not decision.auto_execute:
                ...
                logger.info(
                    f"Decision {decision.decision_id} requires approval, skipping auto-execute"
                )
                continue
    ...
    async def _create_rollback_point(
        self, decision: Decision, context: CognitiveContext
    ) -> Dict[str, Any]:
...
class ProofLayer:
    ...
    def __init__(self):
        ...
        # Statistics
        self._stats = {"evidence_generated": 0, "audit_entries": 0, "verifications": 0}
...
class EnhancedCognitiveProcessor:
    ...
    async def process_signal(
        self, signal: CognitiveSignal, session_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "error": str(e), "signal_id": signal.signal_id}
    ...
    def _get_or_create_context(self, session_id: str) -> CognitiveContext:
        ...
        if session_id not in self._contexts:
            self._contexts[session_id] = CognitiveContext(
                context_id=f"ctx-{uuid4().hex[:8]}", session_id=session_id
            )
