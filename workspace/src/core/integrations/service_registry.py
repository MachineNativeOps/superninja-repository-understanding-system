...
class ServiceRegistry:
    ...
    def register_service(
        ...
        self._safe_emit_event(
            "service_registered",
            {"service_id": service_id, "name": name, "category": category.value},
        )
    ...
    def update_health(
        ...
        # Emit status change event (safely handle case when no event loop is
        # running)
        ...
        for service_id, service in list(self._services.items()):
            ...
            if checker:
                try:
                    ...
                    self.update_health(
                        service_id, ServiceStatus.UNHEALTHY, details={"error": str(e)}
                    )
