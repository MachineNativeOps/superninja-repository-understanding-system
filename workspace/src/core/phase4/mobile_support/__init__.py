...
class MobileAppGenerator:
    ...
    async def generate_mobile_app(
        self, user_input: str, platform: MobilePlatform, framework: str = None
    ) -> Dict[str, Any]:
        """生成移動應用"""
        try:
            ...
            generation_result = await self._generate_app_code(
                app_spec, platform, selected_framework
            )
    ...
    def _analyze_features(self, user_input: str) -> Dict[str, bool]:
        ...
        features = {
            ...
            "offline": any(
                word in user_input_lower for word in ["offline", "no internet", "cache"]
            ),
            "authentication": any(
    ...
    def _analyze_device_requirements(self, user_input: str) -> Dict[str, Any]:
        ...
        # 屏幕方向
        orientations = {
            ScreenOrientation.PORTRAIT: any(
                word in user_input_lower for word in ["portrait", "vertical"]
            ),
            ScreenOrientation.LANDSCAPE: any(
                word in user_input_lower for word in ["landscape", "horizontal"]
            ),
            ScreenOrientation.AUTO: "both" in user_input_lower
            or "any orientation" in user_input_lower,
        }
    ...
    async def _create_app_spec(
        self, user_input: str, platform: MobilePlatform, framework: str
    ) -> MobileAppSpec:
        """創建應用規格"""
        # 提取應用名稱
    ...
    async def _generate_app_code(
        self, app_spec: MobileAppSpec, platform: MobilePlatform, framework: str
...
"""
    ...
    async def _generate_generic_code(
        self, app_spec: MobileAppSpec, platform: MobilePlatform
    ) -> str:
        """生成通用代碼"""
        return """
...
"""
    ...
    def _generate_permissions(
        self, features: Dict[str, bool], platform: MobilePlatform
    ) -> List[str]:
        """生成權限列表"""
        permissions = []
    ...
    def _generate_build_commands(self, platform: MobilePlatform, framework: str) -> List[str]:
        ...
        if platform == MobilePlatform.FLUTTER:
            ...
            commands = ["./gradlew assembleDebug", "./gradlew assembleRelease"]
