...
class MultiLanguageManager:
    ...
    def _initialize_language_support(self) -> Dict[str, LanguageInfo]:
        ...
        return {
            ...
            "Java": LanguageInfo(
                ...
                LanguageCategory.BACKEND,
                ".java",
                ["Spring Boot", "Spring Cloud", "Maven"],
                90,
                4,
                True,
                True,
    ...
    async def generate_code(
        self, user_input: str, language: str, frameworks: List[str]
    ) -> Dict[str, Any]:
        """生成指定語言的代碼"""
        try:
    ...
    def _identify_project_type(self, user_input: str) -> str:
        ...
        return (
            ...
            if any(keyword_scores.values())
            else "web"
        )

    def _recommend_tech_stack(
        self, requirements: Dict[str, Any], project_type: str
    ) -> Dict[str, List[str]]:
        """推薦技術棧"""
        recommendations = {}
    ...
    def _calculate_language_score(
        self, lang_info: LanguageInfo, requirements: Dict[str, Any]
    ) -> float:
        """計算語言適配分數"""
        score = 0.0
    ...
    async def _perform_quality_check(
        self, generation_result: CodeGenerationResult, language: str
    ) -> Dict[str, Any]:
        """執行代碼質量檢查"""
        # 模擬代碼質量檢查
        return {
...
""",
    ...
    async def generate(
        ...
        for framework in frameworks:
            framework_files = await self._generate_framework_files(
    ...
    async def _generate_framework_files(
        self, framework: str, language: str, project_name: str
    ) -> Dict[str, str]:
        """生成框架特定文件"""
        files = {}
        ...
        if framework == "React" and language == "JavaScript":
            ...
            files["settings.py"] = (
                f"# Django settings for {project_name}\nSECRET_KEY = '{project_name.lower()}-key'\n"
            )
