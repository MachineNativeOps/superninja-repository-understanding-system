...
class ModuleLifecycle:
    ...
    def transition_to(
        self, new_state: LifecycleState, actor: str, reason: Optional[str] = None
    ) -> bool:
        """狀態轉換"""
        valid_transitions = {
...
class YAMLModuleDefinition:
    ...
    lifecycle: ModuleLifecycle = field(
        default_factory=lambda: ModuleLifecycle(state=LifecycleState.DRAFT)
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "YAMLModuleDefinition":
        ...
        lifecycle = ModuleLifecycle(
            ...
            approved_at=(
                datetime.fromisoformat(data["lifecycle"]["approved_at"])
                if data["lifecycle"].get("approved_at")
                else None
            ),
