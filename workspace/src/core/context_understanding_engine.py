...
class ContextUnderstandingEngine:
    ...
    def analyze_request(
        self, user_id: str, request: str, additional_context: Optional[dict] = None
    ) -> ContextAnalysis:
        """
    ...
    def _identify_implicit_requirements(
        self, request: str, contexts: list[ContextEntry]
    ) -> list[str]:
    ...
    def _generate_clarifications(
        self, ambiguities: list[str], parsed_intent: ParsedIntent
    ) -> list[str]:
        ...
        for ambiguity in ambiguities:
            if "systems not clearly specified" in ambiguity:
                clarifications.append(
                    "請問是從哪個系統遷移到哪個系統？/ Which systems are involved?"
                )
            elif "Data type" in ambiguity:
                clarifications.append(
                    "請問需要處理什麼類型的數據？/ What type of data needs to be processed?"
                )
            elif "Timing" in ambiguity:
                clarifications.append(
                    "這是一次性操作還是需要定期執行？/ Is this a one-time operation or scheduled?"
                )
            elif "trade-off" in ambiguity:
                clarifications.append(
                    "在速度和數據驗證之間，哪個更優先？/ Which is more important: speed or validation?"
                )
...
""",
    ...
    def add_domain_context(self, user_id: str, domain_knowledge: dict[str, Any]) -> str:
