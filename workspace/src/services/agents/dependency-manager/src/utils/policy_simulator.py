...
class PolicySimulator:
    ...
    def simulate(
        self, analysis: DependencyAnalysis, scenario: SimulationScenario
    ) -> SimulationResult:
        """
        執行單一情境模擬
        ...
        result.breaking_changes_risk = sum(
            1 for u in result.updates_proposed if u.update_type == UpdateType.MAJOR
        )
    ...
    def simulate_all(self, analysis: DependencyAnalysis) -> list[SimulationResult]:
        """
        執行所有預設情境模擬
    ...
    def compare_scenarios(self, results: list[SimulationResult]) -> dict[str, Any]:
        ...
        Returns:
            比較分析
        """
        comparison = {"scenarios": [], "recommendation": None, "analysis": {}}
    ...
    def _simulate_update(self, dep: Dependency, scenario: SimulationScenario) -> Update | None:
        """
        模擬單個依賴的更新
    ...
    def _recommend_scenario(self, results: list[SimulationResult]) -> dict[str, Any]:
        """
        推薦最佳情境
