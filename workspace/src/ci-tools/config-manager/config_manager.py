            ...
            "properties": {
                ...
                "spec": {
                    ...
                    "properties": {
                        ...
                        "selector": {"type": "object", "required": ["matchLabels"]},
                        "template": {
                            "type": "object",
                            "required": ["metadata", "spec"],
                            "properties": {
                                "metadata": {"type": "object", "required": ["labels"]},
                                "spec": {
                                    "type": "object",
                                    "required": ["containers"],
                                    "properties": {
    ...
    def create_config_from_template(
        ...
        try:
            ...
            if template.config_type in [ConfigType.WORKFLOW, ConfigType.DEPLOYMENT]:
                ...
                # 環境變數配置特殊處理
                content = {"raw_content": rendered_content, "variables": variables}
            else:
...
if __name__ == "__main__":
    ...
    ci_config = manager.create_config_from_template(
        "ci-workflow", workflow_vars, "CI Pipeline", "標準 CI 流水線配置"
    )
