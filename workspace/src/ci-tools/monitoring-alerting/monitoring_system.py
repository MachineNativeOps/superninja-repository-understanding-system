...
class MonitoringSystem:
    ...
    def _cleanup_old_metrics(self):
        ...
        # 刪除過期的數據
        self.metric_data = [m for m in self.metric_data if m.timestamp > retention_time]
            ...
            try:
                ...
                if data:
                    ...
                    if not labels or all(
                        metric_info["labels"].get(k) == v for k, v in labels.items()
                    ):
    ...
    async def _send_notification_to_channel(
        self, alert: Alert, rule: AlertRule, channel: NotificationChannel
    ):
        """發送通知到指定渠道"""
    ...
    def start_prometheus_server(self):
        ...
        self.logger.info(
            f"Prometheus 指標服務器已啟動: http://localhost:{port}{self.config['prometheus']['endpoint']}"
        )
