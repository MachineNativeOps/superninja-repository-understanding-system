...
from .models import BusinessStatus, Project, Task
...
class BusinessWorkflowEngine:
    ...
    async def _register_default_workflows(self):
        ...
        await self.workflow_engine.create_workflow(
            WorkflowType.PROJECT_LIFECYCLE.value, "項目生命周期管理", project_workflow
        )

        await self.workflow_engine.create_workflow(
            WorkflowType.TASK_APPROVAL.value, "任務審批流程", task_approval_workflow
        )

        await self.workflow_engine.create_workflow(
            WorkflowType.RESOURCE_ALLOCATION.value, "資源分配流程", resource_workflow
        )

        await self.workflow_engine.create_workflow(
            WorkflowType.QUALITY_CHECK.value, "質量檢查流程", quality_workflow
        )
    ...
    async def execute_project_workflow(self, project_id: str) -> Dict[str, Any]:
        ...
        if execution.status == BusinessStatus.COMPLETED:
            await self.business_service.update_project(
                project_id, {"status": BusinessStatus.ACTIVE}
            )
    ...
    async def execute_resource_allocation_workflow(
        self, project_id: str, resource_requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        """執行資源分配工作流"""
        project = await self.business_service.get_project(project_id)
        if not project:
    ...
    async def execute_custom_workflow(
        self, workflow_id: str, execution_name: str, parameters: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """執行自定義工作流"""
        ...
        execution = await self.business_service.execute_workflow(
            workflow_id, execution_name, await get_current_user(), parameters or {}
        )
