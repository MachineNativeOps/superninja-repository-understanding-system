...
class BusinessServiceManager:
    ...
    async def create_project(self, request: CreateProjectRequest) -> Project:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event(
            "project_created", {"project_id": project.id, "owner": project.owner}
        )
    ...
    async def update_project(self, project_id: str, updates: Dict[str, Any]) -> Optional[Project]:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event("project_updated", {"project_id": project_id})
    ...
    async def delete_project(self, project_id: str) -> bool:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event("project_deleted", {"project_id": project_id})
    ...
    async def create_task(self, request: CreateTaskRequest) -> Task:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event(
            "task_created", {"task_id": task.id, "project_id": task.project_id}
        )
    ...
    async def update_task(self, task_id: str, request: UpdateTaskRequest) -> Optional[Task]:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event(
            "task_updated", {"task_id": task_id, "status": task.status}
        )
    ...
    async def delete_task(self, task_id: str) -> bool:
        ...
        # 觸發核心引擎事件
        await self.core_engine.emit_event(
            "task_deleted", {"task_id": task_id, "project_id": task.project_id}
        )
    ...
    async def get_business_metrics(self) -> BusinessMetrics:
        ...
        completed_tasks_with_time = [
            t
            for t in tasks
            if t.status == BusinessStatus.COMPLETED and t.started_at and t.completed_at
        ]
            ...
            "user_tasks": [
                {
                    ...
                    "progress": t.progress,
                    "project_name": (
                        self.projects[t.project_id].name
                        if t.project_id in self.projects
                        else "Unknown"
                    ),
                }
