...
try:
    ...
    from ..new.security.auth import authenticate, get_current_user
except ImportError:
    # 為測試目的提供模擬實現
...
class BusinessAPI:
    ...
    def _setup_routes(self):
        ...
        async def create_project(
            request: CreateProjectRequest, user_id: str = Depends(get_current_user)
        ):
            """創建項目"""
            try:
        ...
        async def execute_task_approval_workflow(
            task_id: str, user_id: str = Depends(get_current_user)
        ):
            """執行任務審批工作流"""
            try:
        ...
        async def cancel_workflow_execution(
            execution_id: str, user_id: str = Depends(get_current_user)
        ):
            """取消工作流執行"""
            try:
