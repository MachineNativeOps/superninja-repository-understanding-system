...
class EvolutionOrchestrator:
    ...
    def generate_actions_for_objective(self, objective: dict[str, Any]) -> list[RefactorAction]:
        ...
        if obj_id == "language-governance" and score < 100:
            ...
            # Assuming 8 total clusters
            missing_clusters = int((1.0 - coverage) * 8)
    ...
    def check_constraints(self, action: RefactorAction) -> tuple[bool, list[str]]:
        ...
        for constraint in self.constraints:
            ...
            if "safety-critical" in constraint and action.cluster == "autonomous":
                if (
                    "自動修改" in constraint
                    or "automatic modification" in action.description.lower()
                ):
                    violations.append(
    ...
    def export_plan_to_markdown(self, plan: ActionPlan, output_path: Path | None = None) -> str:
        ...
        for priority in ActionPriority:
            ...
            for action in priority_actions:
                ...
                lines.append(
                    f"- **Expected Improvement**: +{action.expected_improvement:.1f} points"
                )
