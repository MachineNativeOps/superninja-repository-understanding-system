...
class IntentUnderstandingEngine:
    ...
    def parse_intent(
        self, text: str, user_id: str = "anonymous", language: str = "auto"
    ) -> BusinessIntent:
        """
    ...
    def _generate_description(
        self, task_type: TaskType, entities: Dict[str, Any], language: str
    ) -> str:
    ...
    def _update_context_memory(self, user_id: str, intent: BusinessIntent) -> None:
        ...
        if user_id not in self.context_memories:
            self.context_memories[user_id] = ContextMemory(
                memory_id=f"mem-{uuid.uuid4().hex[:8]}", user_id=user_id
            )
...
class TaskOrchestrationEngine:
    ...
    def generate_workflow(self, intent: BusinessIntent) -> OrchestratedWorkflow:
        ...
        workflow = OrchestratedWorkflow(
            ...
            metadata={"priority": intent.priority, "language": intent.language},
        )
    ...
    def register_step_handler(
        self, step_name: str, handler: Callable[..., Awaitable[Dict[str, Any]]]
    ) -> None:
