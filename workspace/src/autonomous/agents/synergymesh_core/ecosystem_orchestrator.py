...
class EcosystemOrchestrator:
    ...
    async def _process_message_for_subsystem(
        self, message: EcosystemMessage, subsystem: Subsystem
    ) -> None:
        ...
        if subsystem.handler:
            try:
                ...
                logger.error(f"Error processing message for {subsystem.name}: {e}")
    ...
    async def _execute_coordination(
        self, task: CoordinationTask, steps: List[Dict[str, Any]]
    ) -> None:
    ...
    def allocate_resource(
        ...
        logger.debug(f"Resource allocated: {amount} {resource_type} to {subsystem_id}")
        ...
        while self.is_running:
            try:
                ...
                for subsystem_id, subsystem in self.subsystems.items():
                    ...
                    if (
                        time_diff > self.HEARTBEAT_TIMEOUT_SECONDS
