...
class AutoUpgradeEnvironment:
    ...
    def install_packages_parallel(
        self, packages: List[str], max_workers: int = 3
        ...
        """
        logger.info(
            f"並行安裝 Parallel installation: {len(packages)} packages with {max_workers} workers"
        )
...
if __name__ == "__main__":
    ...
    if args.recommend:
        ...
        if recommendations:
            print(
                f"💡 推薦 {args.recommend} 的相關依賴 Recommended dependencies for {args.recommend}:"
            )
    ...
    if args.from_config:
        ...
        if args.detect_conflicts:
            ...
            if conflicts:
                print(
                    f"⚠ 檢測到 {len(conflicts)} 個版本衝突 Detected {len(conflicts)} version conflicts:\n"
                )
                for conflict in conflicts:
