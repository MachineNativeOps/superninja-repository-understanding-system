...
class TestDelegationManager:
    ...
    @pytest.fixture
    def manager(self):
        config = DelegationConfig(name="test-delegation", max_concurrent_tasks=10)
    ...
    async def test_delegate_task(self, manager):
        ...
        result = await manager.delegate(
            task_name="test-task", task_type="analyze:code", payload={"code": "test"}
        )
    ...
    async def test_get_stats(self, manager):
        ...
        await manager.delegate(task_name="task-1", task_type="test", payload={})
...
class TestCloudProviderAdapter:
    ...
    def aws_adapter(self):
        config = ProviderConfig(
            name="aws-lambda", provider_type=ProviderType.AWS, region="us-east-1"
        )
    ...
    def gcp_adapter(self):
        config = ProviderConfig(
            name="gcp-functions", provider_type=ProviderType.GCP, region="us-central1"
        )
        return CloudProviderAdapter(config)
...
class TestTaskRouter:
    ...
    async def test_route_matching_rule(self, router):
        ...
        task = Task(id="test-123", name="analyze code", type="analyze:code", payload={})
    ...
    async def test_route_default(self, router):
        ...
        task = Task(id="test-456", name="unknown task", type="unknown:type", payload={})
    ...
    def test_add_remove_rule(self, router):
        ...
        new_rule = RoutingRule(
            id="new-rule", name="new", pattern="new:*", preferred_provider="azure"
        )
