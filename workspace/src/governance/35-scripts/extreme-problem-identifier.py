...
class ExtremeProblemIdentifier:
    ...
    def log(self, message: str, level: str = "info"):
        ...
        def redact_sensitive(msg: str) -> str:
            # Remove common possible secret substrings (passwords/keys/tokens)
            # from msg
            redacted = msg
        ...
        def is_potentially_sensitive(msg: str) -> bool:
            ...
            if any(
                s in lowered
                for s in ["password", "api key", "apikey", "secret", "token", "bearer "]
            ):
                return True
        ...
        # As a final safeguard, do not log messages that still look sensitive
        # after redaction
    ...
    def add_problem(self, problem: Problem):
        ...
        # Log based on category and severity - suppress all details for
        # SECURITY problems
        if problem.category == ProblemCategory.SECURITY:
            # Never log sensitive details for security problems, regardless of
            # severity
            self.log(
                ...
                with open(dim_file, "r", encoding="utf-8") as f:
                    ...
                    if data and "metadata" in data:
                        for key in data["metadata"].keys():
                            metadata_fields[key].add(
                                str(dim_file.relative_to(self.governance_root))
                            )
            except Exception:
                pass
    ...
    def print_summary(self, execution_time: float):
        ...
        if critical_count > 0:
            print(
                f"{Colors.FAIL}{Colors.BOLD}❌ RISK LEVEL: CRITICAL - Immediate action required{Colors.ENDC}\n"
            )
        elif high_count > 0:
            print(
                f"{Colors.FAIL}{Colors.BOLD}⚠️  RISK LEVEL: HIGH - Action required soon{Colors.ENDC}\n"
            )
        elif medium_count > 5:
            print(
                f"{Colors.WARNING}{Colors.BOLD}⚠️  RISK LEVEL: MEDIUM - Plan remediation{Colors.ENDC}\n"
            )
        else:
            print(
                f"{Colors.OKGREEN}{Colors.BOLD}✅ RISK LEVEL: LOW - Good governance health{Colors.ENDC}\n"
            )
