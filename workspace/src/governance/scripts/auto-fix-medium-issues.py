            ...
            if isinstance(policy_spec, dict):
                ...
                if current_enforcement != "required":
                    ...
                    if save_yaml_file(dim_file, data):
                        print(
                            f"  {GREEN}✓{RESET} {dim_name}: Changed enforcement optional → required"
                        )
                        fixed_count += 1
                    else:
...
def fix_compliance_frameworks(governance_root: Path) -> int:
    ...
    for dim_file in dim_files:
        ...
        # Ensure compliance section exists under spec (as expected by problem
        # identifier)
        ...
        if (
            "frameworks" not in data["spec"]["compliance"]
            ...
            for framework in frameworks:
                ...
                existing = [
                    m
                    for m in data["spec"]["compliance"]["framework_mappings"]
            ...
            if save_yaml_file(dim_file, data):
                print(
                    f"  {GREEN}✓{RESET} {dim_name}: Added {len(frameworks)} compliance framework(s)"
                )
                fixed_count += 1
            else:
                print(f"  ✗ {dim_name}: Failed to save changes")
