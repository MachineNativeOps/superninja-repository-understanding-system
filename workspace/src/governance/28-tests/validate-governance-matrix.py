...
class GovernanceValidator:
    ...
    def validate_layers_domains(self) -> bool:
        ...
        self.log_info(
            f"✅ Layers & domains defined ({len(data.get('layers', {}))} layers, {len(data.get('domains', {}))} domains)"
        )
        return True
            ...
            if data and "contract" in data:
                ...
                for field in required_fields:
                    if field not in contract:
                        self.log_warning(
                            f"Contract {contract_file.name} missing field: contract.{field}"
                        )
    ...
    def validate_cross_references(self) -> bool:
        ...
        if modules_without_contracts:
            ...
            self.log_info(
                f"{count} modules without behavior contracts (this is expected during rollout)"
            )
            if self.verbose:
