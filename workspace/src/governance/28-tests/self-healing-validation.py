...
class ValidationResult:
    ...
    def add_pass(self, check: str, message: str = ""):
        """添加通過的檢查"""
        self.passed += 1
        self.checks.append({"check": check, "status": "PASS", "message": message})

    def add_fail(self, check: str, message: str):
        """添加失敗的檢查"""
        self.failed += 1
        self.checks.append({"check": check, "status": "FAIL", "message": message})

    def add_warning(self, check: str, message: str):
        """添加警告"""
        self.warnings += 1
        self.checks.append({"check": check, "status": "WARNING", "message": message})
                ...
                with open(engine_path, "r", encoding="utf-8") as f:
                    ...
                    for agent_name, _, _ in agents:
                        if agent_name.replace(" ", "_").lower() in content.lower():
                            ...
                            result.add_warning(
                                f"{agent_name} 未在整合引擎中找到", "可能使用不同命名"
                            )
            ...
            try:
                with open(state_file, "r", encoding="utf-8") as f:
                    ...
                    if "self_healing_integration" in content.lower():
                        ...
                        result.add_fail(
                            "AUTONOMOUS_AGENT_STATE.md 缺少無人島整合狀態",
                            "請更新文件以包含整合狀態",
                        )
            except Exception as e:
