...
class ConsolidatedReportGenerator:
    ...
    def load_governance_report(self):
        ...
        if report_path.exists():
            with open(report_path, encoding="utf-8") as f:
                ...
                print(
                    f"✓ Loaded governance report: {len(self.governance_data.get('violations', []))} violations"
                )
        else:
    ...
    def generate_report(self):
        ...
        if self.governance_data:
            ...
            gov_critical = sum(
                1
                for v in self.governance_data.get("violations", [])
                if v.get("severity") == "CRITICAL"
            )
            total_issues += gov_violations
            critical_issues += gov_critical
            lines.append(
                f"- **Language Governance:** {gov_violations} violations ({gov_critical} critical)"
            )
        ...
        if critical_issues > 0:
            ...
            lines.append(
                "3. Consult the [Language Stack Policy](docs/architecture/language-stack.md)"
            )
            lines.append("")
        ...
        if total_issues > 0:
            ...
            lines.append(
                "3. Request exceptions if necessary through the [governance process](docs/architecture/language-governance.md)"
            )
            lines.append("")
        else:
...
def main():
    ...
    parser.add_argument(
        "--output", default="consolidated-security-report.md", help="Output file path"
    )
