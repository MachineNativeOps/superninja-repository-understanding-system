...
class CIMonitorDashboard:
        ...
        return dbc.Card(
            [
                dbc.CardHeader(
                    [
                        html.H5(
                            [html.I(className="bi bi-table me-2"), "流水線詳細記錄"],
                            className="mb-0",
                        )
                    ]
                ),
                dbc.CardBody([html.Div(id="pipeline-table-container")]),
            ]
        )
    ...
    def setup_callbacks(self):
        ...
        def update_status_distribution(data):
            ...
            fig.update_layout(
                title="流水線狀態分佈", height=300, showlegend=True, font=dict(size=12)
            )
        ...
        def update_quality_score(data):
            ...
            # 根據分數設置顏色
            colors = [
                "#28a745" if score >= 8 else "#ffc107" if score >= 6 else "#dc3545"
                for score in scores
            ]
    ...
    def _create_status_badge(self, status: str) -> dbc.Badge:
        ...
        return dbc.Badge(
            [html.I(className=f"bi bi-{config['icon']} me-1"), config["text"]],
            color=config["color"],
        )
