...
class AxiomPluginManager:
        ...
        with self.lock:
            ...
            try:
                ...
                if init_time > 0.1:  # 100ms limit
                    logger.warning(
                        f"Plugin {plugin_id} init time {init_time:.3f}s exceeds 100ms limit"
                    )
        ...
        try:
            ...
            if plugin_instance:
                ...
                if teardown_time > 0.05:  # 50ms limit
                    logger.warning(
                        f"Plugin {plugin_id} teardown time {teardown_time:.3f}s exceeds 50ms limit"
                    )
    ...
    def execute_plugin(
        self, plugin_id: str, context: dict, timeout: Optional[float] = None
    ...
    def execute_plugin_chain(
        self, plugin_ids: List[str], context: dict, parallel: bool = False
    ...
    def _resolve_execution_order(self, plugin_ids: List[str]) -> List[List[str]]:
        ...
        for plugin_id in plugin_ids:
            if plugin_id in self.dependency_graph:
                subgraph[plugin_id] = [
                    dep for dep in self.dependency_graph[plugin_id] if dep in plugin_ids
                ]
            else:
                subgraph[plugin_id] = []
