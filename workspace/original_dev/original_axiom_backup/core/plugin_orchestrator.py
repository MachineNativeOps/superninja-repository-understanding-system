...
from .plugin_manager import PluginExecutionResult, PluginValidationError, plugin_manager
...
class PluginOrchestrator:
    ...
    def _build_execution_plan(self, plugin_ids: List[str]) -> List[List[str]]:
        ...
        # Group into execution levels (plugins at same level can run in
        # parallel)
    ...
    def _execute_execution_plan(
        self, execution_plan: List[List[str]], context: ExecutionContext
    ) -> List[PluginExecutionResult]:
        ...
        for level_plugins in execution_plan:
            ...
            if context.error_handling == ErrorHandlingStrategy.STOP_ON_FAILURE:
                ...
                if failed_results:
                    logger.error(
                        f"Stopping execution due to failures in plugins: {[r.plugin_id for r in failed_results]}"
                    )
                    break
    ...
    def _execute_level_parallel(
        self, plugins: List[str], context: ExecutionContext
    ) -> List[PluginExecutionResult]:
    ...
    def _execute_level_sequential(
        ...
        for plugin_id in plugins:
            if self._should_execute_plugin(plugin_id, context):
                ...
                if (
                    context.error_handling == ErrorHandlingStrategy.STOP_ON_FAILURE
    ...
    def _execute_plugin_with_retry(
        self, plugin_id: str, context: ExecutionContext
    ) -> PluginExecutionResult:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Plugin {plugin_id} error (attempt {attempt + 1}), retrying in {wait_time}s: {e}"
                    )
                    time.sleep(wait_time)
    ...
    def _group_by_execution_level(
        self, execution_order: List[str], graph: nx.DiGraph
    ) -> List[List[str]]:
