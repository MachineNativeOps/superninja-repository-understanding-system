...
class Plugin:
    ...
    def _encrypt(
        self, source: Path, target: Path, encryption_key: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _decrypt_data(
        self, encrypted_data: bytes, metadata: EncryptionMetadata, key: bytes
    ...
    def _rotate_key(self) -> Dict[str, Any]:
        ...
        for key_id, key_info in self.key_metadata.items():
            if (
                key_info.is_active
                and (time.time() - key_info.created_at) > self.key_rotation_interval
            ):
