...
class Plugin:
    ...
    def initialize(self, config: dict) -> bool:
        ...
        try:
            ...
            logger.info(
                f"ZSTD plugin initialized with level {compression_level}, {threads} threads"
            )
        ...
        with self._lock:
            ...
            if self.metrics.original_size > 0:
                self.metrics.compression_ratio = (
                    self.metrics.compressed_size / self.metrics.original_size
                )

            if self.metrics.processing_time > 0:
                self.metrics.compression_speed = (
                    self.metrics.original_size / (1024 * 1024)
