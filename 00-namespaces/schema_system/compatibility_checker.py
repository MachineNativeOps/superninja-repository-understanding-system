...
class CompatibilityChecker:
    ...
    async def check_compatibility(
        ...
        # 並行執行所有兼容性檢查
        check_tasks = [
            rule(old_schema, new_schema) for rule in self.compatibility_rules.values()
        ]
    ...
    async def generate_migration_guide(
        ...
        compatibility_result = await self.check_compatibility(old_schema, new_schema)
    ...
    def _is_constraint_stricter(
        self, constraint: str, old_value: Any, new_value: Any
    ) -> bool:
        """檢查約束是否變得更嚴格"""
...
async def main():
    ...
    new_schema = {
        ...
        "properties": {
            "name": {"type": "string", "minLength": 1, "maxLength": 100},
            "age": {"type": "integer", "minimum": 18, "maximum": 120},  # 更嚴格的約束
            "phone": {"type": "string"},  # 新增欄位
        },
