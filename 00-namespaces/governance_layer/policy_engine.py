...
class PolicyEngine:
    ...
    def __init__(self):
        ...
        # 事件回調
        self.event_handlers = {"on_policy_violation": [], "on_policy_enforcement": []}
    ...
    async def register_policy(
        ...
        # 緩存
        await self.cache.set(f"policy:{policy_id}", policy.to_dict(), ttl=3600)
    ...
    async def check_permission(
        self, action: str, resource: str, user: str, context: Dict[str, Any] = None
    ) -> bool:
        """
        檢查權限
        ...
        for result in results:
            if result.action == PolicyAction.DENY:
                await self._trigger_event(
                    "on_policy_violation", evaluation_context, result
                )
                return False
    ...
    async def enforce_policy(self, policy_id: str, context: Dict[str, Any]) -> bool:
        ...
        if result:
            await self._trigger_event("on_policy_enforcement", policy_id, result)
...
async def main():
    ...
    await engine.register_policy(
        ...
        "確保所有操作符合 INSTANT 標準",
        [
            {"field": "latency", "operator": "less_than", "value": 500},
            {"field": "autonomy", "operator": "equals", "value": True},
        ],
        PolicyAction.ALLOW,
        priority=100,
    )
    ...
    # 2. 評估政策 - 符合
    context1 = {"action": "create_namespace", "latency": 100, "autonomy": True}
    results = await engine.evaluate(context1)
    print(f"\n✅ 評估結果 (符合):")
    for result in results:
        print(
            f"  {result.policy_id}: {result.action.value} ({'允許' if result.allowed else '拒絕'})"
        )
    ...
    allowed = await engine.check_permission(
        "create_namespace", "platform-registry-service", "system", context1
    )
    print(f"\n✅ 權限檢查: {allowed}")
