...
class AuthManager:
    ...
    def __init__(self, secret_key: str = "instant-secret"):
        ...
        # 統計
        self.stats = {"total_authentications": 0, "successful": 0, "failed": 0}
    ...
    async def authenticate(self, username: str, password: str) -> AuthResult:
        ...
        if username not in self.users:
            ...
            return AuthResult(success=False, error="用戶不存在", latency_ms=latency)
        ...
        if password_hash != user["password_hash"]:
            self.stats["failed"] += 1
            latency = (time.time() - start_time) * 1000
            return AuthResult(success=False, error="密碼錯誤", latency_ms=latency)
        ...
        return AuthResult(
            success=True, user_id=username, token=token, latency_ms=latency
        )
    ...
    def _generate_token(self, user_id: str) -> str:
        ...
        signature = hmac.new(
            self.secret_key.encode(), f"{user_id}:{timestamp}".encode(), hashlib.sha256
        ).hexdigest()
