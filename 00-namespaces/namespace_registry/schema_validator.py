...
class SchemaValidator:
    ...
    async def validate_schema(
        ...
        # 並行執行所有驗證規則
        validation_tasks = [
            rule(schema_id, schema) for rule in self.validation_rules.values()
        ]
    ...
    async def validate_all(
        ...
        # 構建結果字典
        result_dict = {result.schema_id: result for result in results}
    ...
    async def generate_fix(
        self, schema_id: str, schema: Dict[str, Any], errors: List[str]
    ) -> Dict[str, Any]:
        """
        生成修復建議
...
async def main():
    ...
    test_schema = {
        ...
        "version": "1.0.0",
        "properties": {
            "namespace": {"type": "string", "format": "uuid"},
            "metadata": {
                "type": "object",
                "properties": {
