...
class TestDAGMaintenanceAgent:
    ...
    @pytest.mark.asyncio
    async def test_scan_workspace_success(
        self, dag_maintenance_agent, mock_filesystem_scan_result
    ):
        """Test successful workspace scanning"""
        # Mock MCP client response
        dag_maintenance_agent.mcp_client.call_tool.return_value = (
            mock_filesystem_scan_result
        )
        ...
        dag_maintenance_agent.mcp_client.call_tool.assert_called_once_with(
            ...
            tool="scan_directory",
            params={"path": "/workspace", "pattern": "*.yaml", "recursive": True},
        )
    ...
    async def test_scan_workspace_error(self, dag_maintenance_agent):
        ...
        dag_maintenance_agent.mcp_client.call_tool.side_effect = Exception(
            "Scan failed"
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_taxonomy_compliant(
        self, dag_maintenance_agent, sample_dependency_nodes
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_taxonomy_violations(
        self, dag_maintenance_agent, sample_dependency_nodes
    ):
    ...
    async def test_auto_repair_success(self, dag_maintenance_agent):
        ...
        dag_maintenance_agent.taxonomy.validate_and_fix.side_effect = [
            ...
            {
                "fixed": "another-correct-name-v1",
                "changes": ["Removed invalid characters"],
            },
        ]
    ...
    async def test_auto_repair_failure(self, dag_maintenance_agent):
        ...
        violations = ["BadName: Cannot be fixed"]
    ...
    async def test_create_repair_pr(self, dag_maintenance_agent):
        ...
        # Create PR
        result = await dag_maintenance_agent.create_repair_pr(
            repairs, "auto-repair-20240109"
        )
        ...
        assert (
            call_args[1]["params"]["title"] == "Auto-repair: Taxonomy compliance fixes"
        )
