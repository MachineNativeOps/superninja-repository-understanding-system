...
class ContextManager:
    ...
    def get_all(
        self, scope: ContextScope = ContextScope.GLOBAL, context_id: str = "global"
    ) -> Dict[str, Any]:
        """
    ...
    def get_merged_context(
        ...
        with self._lock:
            ...
            if session_id:
                ...
                if user_id:
                    merged.update(
                        self._contexts.get(ContextScope.USER, {}).get(user_id, {})
                    )

                # Add session context
                merged.update(
                    self._contexts.get(ContextScope.SESSION, {}).get(session_id, {})
                )

            # Add invocation context
            if invocation_id:
                merged.update(
                    self._contexts.get(ContextScope.INVOCATION, {}).get(
                        invocation_id, {}
                    )
                )
            ...
            if workflow_id:
                merged.update(
                    self._contexts.get(ContextScope.WORKFLOW, {}).get(workflow_id, {})
                )

            return copy.deepcopy(merged)

    def create_snapshot(self, scope: ContextScope, context_id: str) -> str:
        """
    ...
    def restore_snapshot(self, snapshot_id: str) -> bool:
        ...
        with self._lock:
            ...
            self._contexts[snapshot.scope][snapshot.context_id] = copy.deepcopy(
                snapshot.data
            )
    ...
    def clear_context(
        self, scope: ContextScope, context_id: Optional[str] = None
    ) -> None:
        """
        Clear context data.
    ...
    def export_context(self, scope: ContextScope, context_id: str) -> Dict[str, Any]:
        """
