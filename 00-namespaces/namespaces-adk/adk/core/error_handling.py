...
class ErrorHandler:
    ...
    def register_retry_policy(self, operation_name: str, policy: RetryPolicy) -> None:
        """
    ...
    def get_retry_policy(self, operation_name: str) -> RetryPolicy:
        ...
        return self._retry_policies.get(operation_name, RetryPolicy())  # Default policy
    ...
    async def handle_error(self, error_info: ErrorInfo) -> None:
        ...
        # Publish error event
        if self.event_bus:
            await self.event_bus.publish("error.occurred", error_info.to_dict())
    ...
    def _update_stats(self, error_info: ErrorInfo) -> None:
        ...
        if error_info.component:
            ...
            self._stats["by_component"][component] = (
                self._stats["by_component"].get(component, 0) + 1
            )
