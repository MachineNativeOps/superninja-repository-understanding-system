...
class PluginManager:
    ...
    def __init__(self, directories: List[str], event_bus: Optional[EventBus] = None):
    ...
    async def _discover_plugins(self, directory: Path) -> None:
        ...
        for manifest_path in directory.rglob("manifest.json"):
            try:
                ...
                self.logger.error(
                    f"Failed to load plugin from {manifest_path}: {e}", exc_info=True
                )

    async def _load_plugin_from_manifest(self, manifest_path: Path) -> None:
        ...
        plugin = Plugin(manifest=manifest, module=module, enabled=True)
    ...
    async def unload_plugin(self, plugin_id: str) -> bool:
        ...
        if self.event_bus:
            await self.event_bus.publish("plugin.unloaded", {"plugin_id": plugin_id})
