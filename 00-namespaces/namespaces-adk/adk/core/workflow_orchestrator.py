...
class WorkflowOrchestrator:
    ...
    def get_active_workflow_count(self) -> int:
        ...
        return sum(
            1 for e in self.executions.values() if e.state == WorkflowState.RUNNING
        )
    ...
    async def execute_workflow(
        ...
        execution = WorkflowExecution(
            workflow_id=workflow.id, context={**(context or {}), **(inputs or {})}
        )
        ...
        try:
            ...
            await self._execute_workflow_graph(workflow, graph, execution)
    ...
    async def _execute_workflow_graph(
        ...
        # Get start step
        start_step_id = (
            workflow.start_step or workflow.steps[0].id if workflow.steps else None
        )
...
# SECURITY: eval() used with trusted input only. Do not use with untrusted user input.
    ...
    async def _execute_subtask(
        self, task: Dict[str, Any], execution: WorkflowExecution
    ) -> Any:
    ...
    async def approve_step(
        self, execution_id: str, step_id: str, approved: bool
    ) -> Dict[str, Any]:
        """
    ...
    async def cancel_workflow(self, execution_id: str) -> Dict[str, Any]:
        ...
        await self.event_bus.publish(
            "workflow.cancelled",
            {"workflow_id": execution.workflow_id, "execution_id": execution_id},
        )
