...
class PermissionManager:
    ...
    def _initialize_default_roles(self) -> None:
        ...
        # Admin role
        admin_role = Role(
            role_id="admin", name="Administrator", permissions=set(Permission)
        )
        ...
        # Read-only role
        readonly_role = Role(
            role_id="readonly", name="Read Only", permissions={Permission.READ}
        )
        self._roles[readonly_role.role_id] = readonly_role

    def create_role(
        self, role_id: str, name: str, permissions: List[Permission]
    ) -> Role:
        """Create a role."""
        role = Role(role_id=role_id, name=name, permissions=set(permissions))
            ...
            for policy in self._policies.values():
                ...
                if policy.resource == resource:
                    ...
                    if policy.allowed_roles and (user_role_ids & policy.allowed_roles):
                        if (
                            permission in policy.allowed_permissions
                            or not policy.allowed_permissions
                        ):
                            self._cache[cache_key] = True
                            return True
