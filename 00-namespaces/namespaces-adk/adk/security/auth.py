...
class Authenticator:
    ...
    def verify_password(self, user_id: str, password: str) -> bool:
        ...
        derived = hashlib.pbkdf2_hmac("sha256", password.encode(), salt, 100_000).hex()

        return secrets.compare_digest(derived, stored_hash)

    async def authenticate(
        self, credentials: Dict[str, Any], method: AuthMethod = AuthMethod.TOKEN
        ...
        if method == AuthMethod.PASSWORD:
            ...
            return {"user_id": user.user_id, "roles": user.roles, "session_id": None}
            ...
            if (
                ...
                and datetime.now() >= token_data["expires_at"]
            ):
                return None
    ...
    def generate_token(
        self, user_id: str, session_id: Optional[str] = None, expires_in_hours: int = 24
    ) -> str:
