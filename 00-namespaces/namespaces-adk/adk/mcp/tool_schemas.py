...
class ToolSchema:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "output_schema": (
                self.output_schema.to_json_schema() if self.output_schema else None
            ),
...
class SchemaValidator:
    ...
    def validate_input(
        self, schema: ParameterSchema, data: Dict[str, Any]
        ...
        if schema.enum and value not in schema.enum:
            return (
                False,
                f"Invalid value for {schema.name}: must be one of {schema.enum}",
            )
        ...
        if schema.items and isinstance(value, list):
            for i, item in enumerate(value):
                ...
                is_valid, error = self.validate_input(
                    item_schema, {item_schema.name: item}
                )
                if not is_valid:
                    return False, error
...
class ToolSchemas:
    ...
    def delete_schema(self, tool_name: str, version: str) -> bool:
        """
    ...
    def convert_schema(
        self, schema: ToolSchema, target_format: SchemaFormat
    ) -> Dict[str, Any]:
        """
    ...
    def validate_tool_input(
        self, tool_name: str, input_data: Dict[str, Any], version: Optional[str] = None
    ) -> tuple[bool, Optional[str]]:
        """
