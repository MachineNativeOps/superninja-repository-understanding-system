...
class ToolRouter:
    ...
    def register_endpoint(
        ...
        # Sort by priority
        self._tool_endpoints[tool_name].sort(key=lambda e: e.priority, reverse=True)

        self.logger.debug(f"Registered endpoint: {tool_name}@{server_id}")

    def register_local_tool(self, tool_name: str, handler: Callable) -> None:
        """
    ...
    def unregister_endpoint(self, tool_name: str, server_id: str) -> bool:
        """
        ...
        self._tool_endpoints[tool_name] = [
            e
            for e in self._tool_endpoints[tool_name]
    ...
    def set_policy(self, tool_name: str, policy: RoutingPolicy) -> None:
        """
    ...
    async def route_and_call(
        ...
        try:
            ...
            if tool_name in self._local_tools:
                return await self._call_local_tool(tool_name, arguments, context)
            ...
            if policy.require_permission and self.security:
                if not await self.security.check_tool_permission(tool_name, context):
            ...
            return await self._call_tool_with_retry(
                endpoint, arguments, policy, context
            )
    ...
    def _select_endpoint(
        self, tool_name: str, preferred_server_id: Optional[str], policy: RoutingPolicy
    ...
    async def _call_tool_with_retry(
        ...
        for attempt in range(policy.max_retries):
            try:
                ...
                next_endpoint = self._select_endpoint(endpoint.tool_name, None, policy)
    ...
    async def _call_local_tool(
        ...
        try:
            ...
            return MCPToolResult(
                call_id=str(uuid.uuid4()), success=True, output={"result": output}
            )

        except Exception as e:
            return MCPToolResult(call_id=str(uuid.uuid4()), success=False, error=str(e))
