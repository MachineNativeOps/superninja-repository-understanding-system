...
class CICDRepairAgent:
    ...
    def __init__(
        ...
        self.pipeline_monitor_counter = self.metrics.counter(
            "cicd_pipeline_monitors_total", "Total pipeline monitors", ["status"]
        )
        ...
        self.fix_counter = self.metrics.counter(
            "cicd_fixes_applied_total", "Total fixes applied", ["status", "fix_type"]
        )
    ...
    async def monitor_pipelines(self) -> List[PipelineRun]:
        ...
        try:
            ...
            result = await self.mcp_client.call_tool(
                ...
                params={"status": ["failure", "cancelled"], "limit": 20},
            )
            ...
            for run_data in result.get("workflow_runs", []):
                pipeline_run = PipelineRun(
                    ...
                    completed_at=(
                        datetime.fromisoformat(run_data.get("updated_at"))
    ...
    async def analyze_failure(self, failure: PipelineFailure) -> FailureAnalysis:
        ...
        self.logger.info(
            f"Failure analysis complete: {analysis.root_cause} (confidence: {analysis.confidence})"
        )
    ...
    async def apply_fix(self, fix_plan: FixPlan) -> FixResult:
        ...
        try:
            ...
            self.fix_counter.labels(status="success", fix_type=fix_plan.fix_type).inc()
            ...
            self.fix_counter.labels(status="error", fix_type=fix_plan.fix_type).inc()
    ...
    async def _apply_config_fix(self, fix_plan: FixPlan) -> List[str]:
        ...
        for file_path in fix_plan.files_to_modify:
            ...
            result = await self.mcp_client.call_tool(
                server="filesystem-mcp", tool="read_file", params={"path": file_path}
            )
...
"""
    ...
    async def run_repair_cycle(self):
        ...
        try:
            ...
            failed_runs = [
                run for run in pipeline_runs if run.status == PipelineStatus.FAILURE
            ]
            ...
            for run in failed_runs:
                ...
                self.failure_counter.labels(
                    pipeline_name=run.name, error_type=failure.error_type
                ).inc()
                ...
                if analysis.confidence >= self.autonomy_threshold:
                    ...
                    if fix_result.success:
                        ...
                        self.logger.warning(
                            f"Failed to apply fix: {fix_result.validation_result}"
                        )
                else:
                    self.logger.warning(
