...
from extended_validator import (
    DependencyValidator,
    EndpointValidator,
    ExtendedMCPValidator,
    ExtendedValidationResult,
    ExtendedValidationStatus,
    NamingSpecificationValidator,
    ReferenceValidator,
...
class TestDependencyValidator:
    ...
    def test_validate_dependencies_empty(self):
        ...
        artifact = {"metadata": {"name": "test-artifact"}, "kind": "manifest"}
...
class TestSemanticTypeValidator:
    ...
    def test_validate_semantic_consistency_valid(self):
        ...
        artifact = {
            "metadata": {"name": "test-artifact", "semanticType": "manifest"},
    ...
    def test_validate_semantic_consistency_invalid_type(self):
        ...
        artifact = {
            "metadata": {"name": "test-artifact", "semanticType": "invalid-type"},
            "apiVersion": "mcp.io/v1",
            "kind": "Manifest",
        }
    ...
    def test_validate_semantic_relationships(self):
        ...
        artifact = {
            "metadata": {"name": "test-manifest", "semanticType": "manifest"},
            "apiVersion": "mcp.io/v1",
            "kind": "Manifest",
            "spec": {},
            "dependsOn": [
...
class TestEndpointValidator:
    ...
    def test_validate_endpoints_success(self, mock_get):
        ...
        artifact = {
            "metadata": {"name": "test-artifact"},
            "endpoints": [{"name": "api-endpoint", "url": "/api/v1/status"}],
        }
    ...
    def test_validate_endpoints_failure(self, mock_get):
        ...
        artifact = {
            "metadata": {"name": "test-artifact"},
            "endpoints": [{"name": "api-endpoint", "url": "/api/v1/notfound"}],
        }
    ...
    def test_validate_endpoints_timeout(self, mock_get):
        ...
        artifact = {
            "metadata": {"name": "test-artifact"},
            "endpoints": [{"name": "slow-endpoint", "url": "/api/v1/slow"}],
        }
...
class TestNamingSpecificationValidator:
    ...
    def test_validate_artifact_name_reserved(self):
        ...
        artifact = {"metadata": {"name": "mcp", "version": "1.0.0"}}  # Reserved word
...
class TestExtendedMCPValidator:
    ...
    def setup_method(self):
        ...
        self.test_artifact = {
            ...
            "spec": {"description": "Test artifact"},
            "dependsOn": [
                {
    ...
    def test_validate_artifact_extended_success(
        self, mock_exists, mock_yaml, mock_open
    ):
    ...
    @patch("yaml.safe_load")
    @patch("pathlib.Path.exists")
    def test_validate_artifact_extended_with_endpoint_validation(
        self, mock_exists, mock_yaml, mock_open
    ):
        ...
        artifact_with_endpoints["endpoints"] = [
            {"name": "status-endpoint", "url": "/api/v1/status"}
        ]
        ...
        with patch("requests.get") as mock_get:
            ...
            result = self.validator.validate_artifact_extended(
                artifact_path, validate_endpoints=True
            )
    ...
    @patch("yaml.safe_load")
    @patch("pathlib.Path.exists")
    def test_validate_artifact_extended_strict_mode(
        self, mock_exists, mock_yaml, mock_open
    ):
        ...
        artifact_with_warnings["metadata"]["name"] = "mcp"

        # Mock file operations
        mock_exists.return_value = True
        mock_yaml.return_value = artifact_with_warnings
        mock_open.return_value.__enter__.return_value.read.return_value = yaml.dump(
            artifact_with_warnings
        )
