...
import requests
import yaml
from validator import MCPValidator, ValidationResult, ValidationStatus
...
class DependencyValidator:
        ...
        for i, dep in enumerate(dependencies):
            ...
            if "artifact" in dep:
                self.dependency_graph.add_edge(
                    artifact.get("metadata", {}).get("name", "unknown"), dep["artifact"]
                )
    ...
    def _validate_single_dependency(
        self, dep: Dict[str, Any], index: int
    ) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in dep:
                ...
                result["errors"].append(
                    f"Dependency {index} missing required field: {field}"
                )
        ...
        if "artifact" in dep and self.artifact_registry:
            ...
            if artifact_name not in self.artifact_registry:
                ...
                result["warnings"].append(
                    f"Dependency artifact not found in registry: {artifact_name}"
                )
...
class SemanticTypeValidator:
    ...
    def validate_semantic_consistency(self, artifact: Dict[str, Any]) -> Dict[str, Any]:
        ...
        schema_violations = self._validate_against_semantic_schema(
    ...
    def _validate_against_semantic_schema(
        self, artifact: Dict[str, Any], semantic_type: str
    ) -> List[str]:
        ...
        for field in schema.get("required_fields", []):
            if field not in artifact:
                violations.append(
                    f"Missing required field for {semantic_type}: {field}"
                )

        # Check for disallowed fields (basic check)
        allowed_fields = set(
            schema.get("required_fields", []) + schema.get("optional_fields", [])
        )
    ...
    def _validate_semantic_relationships(
        ...
        violations = []
        allowed_dependencies = self.semantic_type_schemas.get(semantic_type, {}).get(
            "can_depend_on", []
        )
...
class EndpointValidator:
    ...
    def validate_endpoints(
        self, artifact: Dict[str, Any], base_url: str = "http://localhost:8080"
    ) -> Dict[str, Any]:
        ...
        for endpoint in endpoints:
            ...
            result["endpoint_status"][endpoint.get("name", "unknown")] = (
                endpoint_result["status"]
            )

            if endpoint_result["response_time"]:
                result["response_times"][endpoint.get("name", "unknown")] = (
                    endpoint_result["response_time"]
                )
    ...
    def _validate_single_endpoint(
        ...
        try:
            ...
            if response.status_code >= 400:
                result["valid"] = False
                result["errors"].append(
                    f"Endpoint returned error status: {response.status_code}"
                )
            ...
            try:
                ...
                if not isinstance(json_response, dict):
                    result["warnings"].append(
                        "Endpoint response is not a valid JSON object"
                    )
...
class NamingSpecificationValidator:
    ...
    def validate_naming_specifications(
        ...
        if "namingRegistry" in metadata:
            namespace_result = self._validate_naming_registry(
    ...
    def _load_reserved_namespaces(self) -> Set[str]:
        ...
        return {"io.github.machinenativeops", "com.mcp", "org.mcp", "net.mcp"}

    def _validate_naming_registry(
        self, naming_registry: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            if re.match(pattern, namespace):
                ...
                if pattern_info.get("verification_required"):
                    if "verificationStatus" not in naming_registry:
                        result["warnings"].append(
                            f"Namespace {namespace} requires verification"
                        )
                    elif naming_registry.get("verificationStatus") != "verified":
                        result["warnings"].append(
                            f"Namespace {namespace} is not verified"
                        )
                break
    ...
    def _validate_artifact_name(self, name: str) -> Dict[str, Any]:
        ...
        if len(name) > max_len:
            result["valid"] = False
            result["errors"].append(
                f"Artifact name too long (max {max_len} characters)"
            )
        if len(name) < min_len:
            result["valid"] = False
            result["errors"].append(
                f"Artifact name too short (min {min_len} characters)"
            )
...
class ReferenceValidator:
    ...
    def _extract_references(
        self, artifact: Dict[str, Any], path: str = ""
    ) -> List[Dict[str, Any]]:
    ...
    def _validate_reference(self, reference: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.artifact_registry and ref_value not in self.artifact_registry:
            result["warnings"].append(
                f"Reference target not found in registry: {ref_value}"
            )
    ...
    def _is_valid_reference_format(self, reference: str) -> bool:
        ...
        # Expected format: namespace/artifact-name or
...
class ExtendedMCPValidator(MCPValidator):
    ...
    def __init__(
        ...
        artifact_registry: Optional[Dict] = None,
    ):
    ...
    def validate_artifact_extended(
        self,
        artifact_path: Path,
        strict: bool = False,
        ...
        if validate_endpoints:
            endpoint_results = self.endpoint_validator.validate_endpoints(
                artifact, base_url
            )
        ...
        return ExtendedValidationResult(
            ...
            circular_dependency_detected=bool(
                dependency_results.get("circular_dependencies")
            ),
...
Examples:
  ...
  extended-validator --artifact manifest.yaml --format json --show-metrics
    ...
    parser.add_argument(
        "--artifact", type=Path, required=True, help="Path to artifact file to validate"
    )

    parser.add_argument("--schema", type=Path, help="Path to schema.yaml file")

    parser.add_argument("--registry", type=Path, help="Path to artifact registry file")
    ...
    parser.add_argument(
        "--validate-endpoints", action="store_true", help="Validate MCP endpoints"
    )
    ...
    parser.add_argument(
        "--show-metrics", action="store_true", help="Show performance metrics"
    )
    ...
    validator = ExtendedMCPValidator(
        schema_path=args.schema, artifact_registry=artifact_registry
    )
