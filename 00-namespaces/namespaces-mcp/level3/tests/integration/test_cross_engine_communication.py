    ...
    async def test_rag_entity_extraction_flow(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                assert "quantum computing" in [
                    e["text"].lower() for e in result["entities"]
                ]
            ...
            while time.time() - start_time < max_wait:
                async with session.get(
                    ...
                    if status["state"] == "completed":
                        ...
                        assert task_order.index("extract") < task_order.index(
                            "transform"
                        )
...
class TestGovernanceIntegration:
    ...
    async def test_governance_policy_enforcement(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            policy = {
                ...
                "type": "rate_limit",
                "rules": [{"resource": "rag-engine", "max_requests_per_minute": 10}],
            }

            async with session.post(
                "http://governance-engine:8080/api/v1/policies", json=policy
            ) as response:
                assert response.status == 201
    ...
    async def test_governance_audit_logging(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                "http://governance-engine:8080/api/v1/audit/logs", params={"limit": 10}
            ) as response:
                assert response.status == 200
...
class TestPromotionRegistryIntegration:
    ...
    async def test_artifact_promotion_flow(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                "http://promotion-engine:8080/api/v1/promotions", json=promotion_request
            ) as response:
                assert response.status == 201
...
class TestEndToEndFlow:
    ...
    async def test_complete_rag_pipeline(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                "http://registry:8080/api/v1/artifacts", json=document
            ) as response:
                doc_id = (await response.json())["artifact_id"]

            # Step 2: Index document (triggers Taxonomy for entity extraction)
            async with session.post(
                "http://rag-engine:8080/api/v1/index", json={"artifact_id": doc_id}
            ) as response:
                assert response.status == 202
                index_job_id = (await response.json())["job_id"]
