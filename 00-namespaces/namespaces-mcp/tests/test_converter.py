...
from converter import ConversionRule, MachineNativeConverter
...
class TestMachineNativeConverter(unittest.TestCase):
    ...
    def test_should_process_file(self):
        ...
        self.assertTrue(self.converter._should_process_file(py_file, ["source_code"]))

        # 日誌文件不應該被處理
        log_file = Path("test.log")
        self.assertFalse(self.converter._should_process_file(log_file, ["source_code"]))
    ...
    def test_ssot_registration(self):
        ...
        self.converter._register_ssot_change(test_file, context, changes, confidence)

        self.assertIn(str(test_file), self.converter.ssot_registry)
        self.assertEqual(len(self.converter.ssot_registry[str(test_file)]), 1)

    def test_ssot_hash_generation(self):
        """測試 SSOT 哈希生成"""
        # 註冊一些變更
        self.converter._register_ssot_change(Path("test1.py"), "context1", 5, 0.9)
        self.converter._register_ssot_change(Path("test2.py"), "context2", 3, 0.8)
