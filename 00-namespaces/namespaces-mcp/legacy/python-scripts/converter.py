...
class MachineNativeConverter:
    ...
    def _build_governance_rules(self) -> List[ConversionRule]:
        """構建治理規則"""
        license_header = self.governance_rules.get("license", {}).get(
            "header_template", ""
        )
    ...
    def _apply_pattern_replacement(
    ...
    def _register_ssot_change(
        self, file_path: Path, context: str, changes: int, confidence: float
    ):
        """註冊 SSOT 變更"""
    ...
    def _generate_conversion_report(
        self, results: Dict[str, ConversionResult], target_dir: Path
    ):
        """生成轉換報告"""
        report = {
            "version": "1.0.0",
