...
export class InstantExecutionEngine extends EventEmitter {
  ...
  async routeEvent(event: AgentTask): Promise<void> {
    ...
    this.tracer.startSpan('event_routing', { taskId: event.id });
    ...
    try {
      ...
      this.tracer.finishSpan('event_routing');
    }
  }
...
class Agent {
  ...
  async execute(task: AgentTask): Promise<any> {
    this.tracer.startSpan('agent_execution', { agentId: this.id, taskId: task.id });
    ...
    try {
      ...
      this.tracer.finishSpan('agent_execution');
    }
  }
