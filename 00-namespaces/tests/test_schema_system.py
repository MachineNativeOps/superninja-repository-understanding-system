...
import pytest
from schema_system.compatibility_checker import (
...
class TestSchemaRegistry:
    ...
    async def test_register_schema(self, registry, valid_schema):
        """測試註冊 schema"""
        result = await registry.register_schema("test-schema", "1.0.0", valid_schema)
    ...
    async def test_register_schema_invalid(self, registry):
        ...
        result = await registry.register_schema("test-schema", "1.0.0", invalid_schema)
    ...
    async def test_update_schema(self, registry, valid_schema):
        ...
        result = await registry.update_schema("test-schema", "1.0.0", updated_schema)
...
class TestSchemaVersioning:
    ...
    async def test_create_version_patch(self, versioning):
        ...
        new_version = await versioning.create_version(
            "test-schema", "1.0.0", ["Fix validation bug"], "developer1"
        )
    ...
    async def test_create_version_minor(self, versioning):
        """測試創建次版本"""
        new_version = await versioning.create_version(
            "test-schema", "1.0.0", ["Add new field"], "developer2"
        )
    ...
    async def test_create_version_major(self, versioning):
        """測試創建主版本"""
        new_version = await versioning.create_version(
            "test-schema", "1.0.0", ["Break change: Remove field"], "architect"
        )
...
class TestCompatibilityChecker:
    ...
    async def test_check_compatibility_compatible(
        self, checker, old_schema, new_schema_compatible
    ):
        """測試兼容的 schema"""
        result = await checker.check_compatibility(old_schema, new_schema_compatible)
    ...
    async def test_check_compatibility_incompatible(
        self, checker, old_schema, new_schema_incompatible
    ):
        """測試不兼容的 schema"""
        result = await checker.check_compatibility(old_schema, new_schema_incompatible)
    ...
    async def test_check_backward_compatibility(
        ...
        compatible = await checker.check_backward_compatibility(
            old_schema, new_schema_compatible
        )
    ...
    async def test_generate_migration_guide(
        ...
        guide = await checker.generate_migration_guide(
            old_schema, new_schema_incompatible
        )
