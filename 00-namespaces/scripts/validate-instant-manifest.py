...
class InstantManifestValidator:
    ...
    def validate_closed_loop_pipeline(self) -> ValidationResult:
        ...
        if pipeline.get("type") != "CLOSED_LOOP":
            details.append(f"❌ pipeline.type 必須為 CLOSED_LOOP")
            all_passed = False
        else:
        ...
        if pipeline.get("mode") != "CONTINUOUS":
            details.append(f"❌ pipeline.mode 必須為 CONTINUOUS")
            all_passed = False
        else:
...
def main():
    ...
    if len(sys.argv) < 2:
        manifest_path = (
            "machine-native-ops/00-namespaces/docs/INSTANT-EXECUTION-MANIFEST.yaml"
        )
    else:
