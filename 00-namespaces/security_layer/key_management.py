...
class KeyManagement:
    ...
    async def create_key(self, key_id: str, ttl_hours: int = 24) -> str:
        """
        創建密鑰
        ...
        # 計算過期時間
        expires_at = (
            datetime.now() + timedelta(hours=ttl_hours) if ttl_hours > 0 else None
        )
    ...
    async def rotate_key(self, key_id: str, ttl_hours: int = 24) -> Optional[str]:
        """
        輪換密鑰
