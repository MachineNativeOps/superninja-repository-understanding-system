        ...
        if duplicates:
            results['analysis']['potential_duplicates'][f'depth_{max_depth}'] = {
                ...
                'directories': {
                    name: {
                        'count': count,
                        'locations': dir_locations[name][:5]  # Show first 5 locations
                    }
...
def main():
    ...
    if similar_pairs:
        ...
        for pair in similar_pairs:
            ...
            print(f"    Location 1: {Path(pair['location1']).relative_to(project_path)}")
            print(f"    Location 2: {Path(pair['location2']).relative_to(project_path)}")
    else:
