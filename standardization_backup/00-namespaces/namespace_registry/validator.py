...
class RegistryValidator:
    ...
    async def validate_namespace(
        ...
        # 並行執行所有驗證規則
        validation_tasks = [
            rule(namespace, data) for rule in self.validation_rules.values()
        ]
    ...
    async def validate_all(
        ...
        # 構建結果字典
        result_dict = {result.namespace: result for result in results}
    ...
    def _check_naming_format(self, namespace: str) -> bool:
        ...
        if len(parts) == 1:
            return bool(namespace.strip()) and bool(
                re.match(r"^[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*$", namespace)
            )
        return len(parts) >= 3
