...
class TestAgentWorkflowIntegration:
    ...
    async def test_multi_agent_coordination(
        self, dag_maintenance_agent, cicd_repair_agent
    ):
...
class TestAgentObservability:
    ...
    async def test_logging_output(self, caplog, dag_maintenance_agent):
        ...
        assert any(
            "Starting maintenance cycle" in record.message for record in caplog.records
        )
    ...
    async def test_error_logging(self, caplog, dag_maintenance_agent):
        ...
        assert any(
            "Error in maintenance cycle" in record.message for record in caplog.records
        )
