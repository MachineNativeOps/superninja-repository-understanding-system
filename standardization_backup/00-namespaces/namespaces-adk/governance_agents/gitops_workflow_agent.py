...
class GitOpsPRWorkflowAgent:
    ...
    def __init__(
        ...
        self.pr_creation_counter = self.metrics.counter(
            "gitops_pr_creations_total", "Total PR creations", ["status"]
        )
        self.pr_review_counter = self.metrics.counter(
            "gitops_pr_reviews_total", "Total PR reviews", ["result"]
        )
        self.policy_enforcement_counter = self.metrics.counter(
            "gitops_policy_enforcements_total", "Total policy enforcements", ["result"]
        )
        self.deployment_counter = self.metrics.counter(
            "gitops_deployments_total", "Total deployments", ["status"]
        )
        self.pr_duration = self.metrics.histogram(
            "gitops_pr_duration_hours", "PR lifecycle duration", ["status"]
        )
    ...
    async def create_pr(self, spec: PRSpec) -> PullRequest:
        ...
        try:
            ...
            title_validation = self.taxonomy.validate_agent_name(
                spec.title.split(":")[0].strip()
            )
            if not title_validation.get("valid", True):
                self.logger.warning(
                    f"PR title does not follow taxonomy: {title_validation}"
                )
            ...
            if not branch_validation.get("valid", True):
                self.logger.warning(
                    f"Branch name does not follow taxonomy: {branch_validation}"
                )
    ...
    async def review_pr(self, pr: PullRequest) -> ReviewResult:
        ...
        try:
            ...
            if (
                not pr.description
                or len(pr.description) < self.policies["min_description_length"]
            ):
                comments.append(
            ...
            if pr.additions + pr.deletions > 1000:
                suggestions.append(
                    "Consider splitting this into smaller PRs for better reviewability"
                )
            ...
            approved = score >= 0.8 and len(issues) == 0 and pr.checks_passed
            ...
            self.pr_review_counter.labels(
                result="approved" if approved else "rejected"
            ).inc()
            self.logger.info(
                f"Review complete: {score:.2f} - {'Approved' if approved else 'Changes requested'}"
            )
        ...
        try:
            ...
            if pr.approvals < self.policies["min_approvers"]:
                ...
                required_actions.append(
                    f"Get {self.policies['min_approvers'] - pr.approvals} more approval(s)"
                )
            ...
            if (
                self.policies["allowed_authors"]
                and pr.author not in self.policies["allowed_authors"]
            ):
                violations.append(
                    f"Author '{pr.author}' is not in allowed authors list"
                )
    ...
    async def coordinate_approval(self, pr: PullRequest) -> ApprovalStatus:
        ...
        try:
            ...
            if pr.approvals >= self.policies["min_approvers"]:
                ...
                self.logger.info(
                    f"PR pending approval ({pr.approvals}/{self.policies['min_approvers']})"
                )
    ...
    async def track_deployment(self, pr: PullRequest) -> DeploymentStatus:
        ...
        try:
            ...
            status = DeploymentStatus(
                ...
                completed_at=(
                    datetime.fromisoformat(result.get("updated_at"))
    ...
    async def manage_pr_lifecycle(self):
        ...
        try:
            # Get open PRs
            result = await self.mcp_client.call_tool(
                server="github-mcp", tool="list_pulls", params={"state": "open"}
            )
            ...
            for pr_data in result.get("pulls", []):
                pr = PullRequest(
                    ...
                    approval_status=ApprovalStatus(
                        pr_data.get("approval_status", "pending")
                    ),
