...
class AgentRuntime:
    ...
    def __init__(self, config: AgentConfig):
        ...
        self.memory_manager = MemoryManager(
            backend=config.memory_backend, event_bus=self.event_bus
        )
        ...
        self.plugin_manager = PluginManager(
            directories=config.plugin_directories, event_bus=self.event_bus
        )
    ...
    def _init_metrics(self):
        ...
        if self.metrics:
            ...
            self.metrics.register_gauge("agent_state", "Current agent state")
        ...
        with self.logger.context(f"request.{request_type}"):
            ...
            try:
                ...
                await self.event_bus.publish(
                    "error.occurred",
                    {"request_type": request_type, "error": str(e), "context": context},
                )
    ...
    async def _handle_workflow_execute(
        ...
        if not workflow_id and not workflow_def:
            return {
                "success": False,
                "error": "workflow_id or workflow_definition required",
            }
    ...
    async def _handle_memory_query(
        ...
        try:
            results = await self.memory_manager.query(
                query_text=query, session_id=session_id, limit=payload.get("limit", 10)
            )
...
def create_runtime(config_path: Optional[str] = None) -> AgentRuntime:
    ...
    """
    if config_path:
        config = AgentConfig(name="default_agent", config_path=config_path)
    else:
