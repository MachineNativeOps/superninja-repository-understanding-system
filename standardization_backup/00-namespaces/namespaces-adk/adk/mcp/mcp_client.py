...
class HTTPTransport(MCPTransport):
    ...
    async def send_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            self.config.url, json=request, timeout=timeout
        ) as response:
...
class MCPClient:
    ...
    def __init__(
        self, servers: Dict[str, MCPServerConfig], tracer: Optional[Tracer] = None
    ):
    ...
    async def connect_server(self, server_id: str, config: MCPServerConfig) -> None:
    ...
    async def _discover_tools(self, server_id: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to discover tools from server {server_id}: {e}", exc_info=True
            )
    ...
    async def call_tool(self, tool_call: MCPToolCall) -> MCPToolResult:
        ...
        try:
            ...
            result.error = str(e)
            self.logger.error(
                f"Tool call failed: {tool_call.tool_name}: {e}", exc_info=True
            )
