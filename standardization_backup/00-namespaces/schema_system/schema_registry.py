...
class SchemaRegistry:
    ...
    async def register_schema(
        ...
        # 5. 緩存
        await self.cache.set(f"schema:{schema_id}@{version}", entry.to_dict(), ttl=3600)

        # 6. 緩存最新版本
        await self.cache.set(f"schema:{schema_id}:latest", entry.to_dict(), ttl=3600)
        ...
        if schema_id in self.version_history:
            ...
            if latest.version == version:
                await self.cache.set(
                    f"schema:{schema_id}:latest", entry.to_dict(), ttl=3600
                )
    ...
    async def delete_schema(
        ...
        if version is None:
            ...
            if schema_id in self.version_history:
                self.version_history[schema_id] = [
                    e for e in self.version_history[schema_id] if e.version != version
                ]
                ...
                if self.version_history[schema_id]:
                    latest = self.version_history[schema_id][-1]
                    await self.cache.set(
                        f"schema:{schema_id}:latest", latest.to_dict(), ttl=3600
                    )
                else:
                    await self.cache.delete(f"schema:{schema_id}:latest")
    ...
    async def validate_schema(
        self, schema_id: str, version: str, data: Dict[str, Any]
    ) -> bool:
        """
        使用 Schema 驗證數據
    ...
    async def _validate_schema_structure(self, schema: Dict[str, Any]) -> bool:
        """驗證 Schema 結構"""
        # 基本結構驗證
...
async def main():
    ...
    await registry.register_schema(
        "namespace-schema", "1.0.0", test_schema, {"owner": "platform-team"}
    )
