...
class SchemaVersioning:
    ...
    async def create_version(
        ...
        print(
            f"✅ 創建版本: {current_version} → {new_version} ({change_type.value})，延遲: {latency:.2f}ms"
        )
    ...
    async def check_compatibility(
        ...
        if v1.major != v2.major:
            ...
            latency = (time.time() - start_time) * 1000
            print(
                f"❌ 不兼容: {from_version} → {to_version} (主版本號不同)，延遲: {latency:.2f}ms"
            )
            return False
    ...
    async def get_version_history(
        self, schema_id: str, limit: int = 10
    ) -> List[VersionChange]:
        """
        獲取版本歷史
    ...
    async def compare_versions(self, version1: str, version2: str) -> str:
        """
        比較兩個版本
    ...
    async def suggest_next_version(
        self, schema_id: str, current_version: str, changes: List[str]
    ) -> str:
        """
        建議下一個版本號
        ...
        # 計算下一版本
        next_version = await self._calculate_next_version(current_version, change_type)

        latency = (time.time() - start_time) * 1000
        print(
            f"✅ 建議版本: {current_version} → {next_version} ({change_type.value})，延遲: {latency:.2f}ms"
        )

        return next_version

    async def _analyze_change_type(self, changes: List[str]) -> VersionChangeType:
        """分析變更類型"""
        # 破壞性變更關鍵詞
        breaking_keywords = [
            "break",
        ...
        # 修補關鍵詞
        patch_keywords = ["fix", "bug", "patch", "update", "correct", "improve"]
    ...
    async def _calculate_next_version(
        self, current_version: str, change_type: VersionChangeType
    ) -> str:
        """計算下一個版本號"""
        try:
...
async def main():
    ...
    v1 = await versioning.create_version(
        schema_id, "0.0.0", ["Initial version"], "system"
    )

    # 2. 創建補丁版本
    v2 = await versioning.create_version(
        schema_id, v1, ["Fix validation bug"], "developer1"
    )

    # 3. 創建次版本
    v3 = await versioning.create_version(
        schema_id, v2, ["Add new field: description"], "developer2"
    )

    # 4. 創建主版本
    v4 = await versioning.create_version(
        schema_id, v3, ["Break change: Remove deprecated field"], "architect"
    )
    ...
    for change in history:
        print(
            f"  {change.from_version} → {change.to_version} ({change.change_type.value})"
        )
    ...
    suggested = await versioning.suggest_next_version(
        schema_id, latest, ["Add new feature"]
    )
    print(f"\n💡 建議下一版本: {suggested}")
