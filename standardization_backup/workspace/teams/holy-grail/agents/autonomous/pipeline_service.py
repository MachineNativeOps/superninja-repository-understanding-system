...
class PipelineService:
    ...
    async def _handle_code_analysis(
        self, query: str, code: str, analysis_type: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _handle_visualization(
        ...
        return {"type": "explanation", "explanation": explanation}
...
"""
    ...
    result = await service.process_request(
        query="分析這段代碼", editor_code=test_code, analysis_type="comprehensive"
    )
    ...
    result = await service.process_request(query="什麼是時間複雜度？")
    print(json.dumps(result, indent=2, ensure_ascii=False))
    print("\n" + "=" * 50 + "\n")
