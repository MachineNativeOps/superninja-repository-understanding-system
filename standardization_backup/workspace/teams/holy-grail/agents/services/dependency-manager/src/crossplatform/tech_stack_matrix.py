...
class TechStackMatrix:
    ...
    def compare_stacks(self, stacks: list[StackRecommendation]) -> dict[str, Any]:
        ...
        comparison = {
            ...
            "highest_scalability": max(
                stacks, key=lambda s: self._scalability_to_score(s.scalability_rating)
            ).to_dict(),
    ...
    def generate_stack_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_compatibility": (
                    sum(r.compatibility_score for r in self.recommendations)
                    / len(self.recommendations)
                    if self.recommendations
                    else 0
                ),
                "most_common_frontend": self._get_most_common(
                    [r.frontend.value for r in self.recommendations]
                ),
                "most_common_backend": self._get_most_common(
                    [r.backend.value for r in self.recommendations]
                ),
            },
        }

    def _get_most_common(self, items: list[str]) -> str | None:
        """獲取最常見項目"""
        if not items:
            return None
