...
class SafetyNet:
    ...
    async def execute(self, operation: Callable[[T], Any], data: T) -> Any:
        """
        ...
        if failures:
            raise SafetyCheckError(
                f"Safety check failed: {failures[0].check_name}", failures=failures
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        enabled_checks = sum(
            len([c for c in checks if c.enabled]) for checks in self._checks.values()
        )
