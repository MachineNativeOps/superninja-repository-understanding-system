...
from ..agents import AgentResult, AgentTask, AgentType, BaseAgent, create_agent
...
class InstantGenerationWorkflow:
    ...
    async def execute_instant_generation(
        ...
        try:
            ...
            workflow = self.orchestrator.create_workflow(
                workflow_id=workflow_id, name="Instant Generation Workflow"
            )
    ...
    def _create_instant_generation_tasks(
    ...
    def _prepare_task_input(
        self, task: WorkflowTask, previous_results: Dict[str, AgentResult]
    ) -> Dict[str, Any]:
        """準備任務輸入數據"""
