...
from .engine import DebugSession, DebugState, get_engine
...
class ErrorAnalyzer:
    ...
    def analyze(
        self, error_type: str, error_message: str, stack_trace: List[str]
    ) -> Dict[str, Any]:
        """分析錯誤"""
        analysis = {
...
class ChatDebugInterface:
    ...
    async def _handle_show_variables(self, intent: Intent) -> str:
        ...
        var_list = "\n".join(
            [f"  • {var.name} = {var.value} ({var.type})" for var in variables[:10]]
        )  # 限制顯示數量
